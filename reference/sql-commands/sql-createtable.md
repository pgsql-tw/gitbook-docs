...
<a id="id-1.9.3.85.6"></a>

## Parameters

<a id="SQL-CREATETABLE-TEMPORARY"></a>
...
:   If specified, the table is created as a temporary table.
    ...
    The [autovacuum daemon](../../server-administration/maintenance/routine-vacuuming.md#AUTOVACUUM) cannot
    ...
    [Compatibility](sql-createtable.md#SQL-CREATETABLE-COMPATIBILITY) below.
<a id="SQL-CREATETABLE-UNLOGGED"></a>

`UNLOGGED` [#](#SQL-CREATETABLE-UNLOGGED)
:   If specified, the table is created as an unlogged table. Data written
    to unlogged tables is not written to the write-ahead log (see [Chapter 28](../../server-administration/wal/README.md)), which makes them considerably faster than ordinary
...
<a id="SQL-CREATETABLE-PARMS-IF-NOT-EXISTS"></a>
...
:   Do not throw an error if a relation with the same name already exists.
    ...
    created.
<a id="SQL-CREATETABLE-PARMS-TABLE-NAME"></a>

*`table_name`* [#](#SQL-CREATETABLE-PARMS-TABLE-NAME)
:   The name (optionally schema-qualified) of the table to be created.
<a id="SQL-CREATETABLE-PARMS-TYPE-NAME"></a>
...
:   Creates a *typed table*, which takes its
    ...
    one created using [CREATE TYPE](sql-createtype.md)) though it still
...
<a id="SQL-CREATETABLE-PARMS-COLUMN-NAME"></a>

*`column_name`* [#](#SQL-CREATETABLE-PARMS-COLUMN-NAME)
:   The name of a column to be created in the new table.
<a id="SQL-CREATETABLE-PARMS-DATA-TYPE"></a>
...
<a id="SQL-CREATETABLE-PARMS-COLLATE"></a>
...
`STORAGE { PLAIN | EXTERNAL | EXTENDED | MAIN | DEFAULT }` <a id="id-1.9.3.85.6.2.9.1.2"></a> [#](#SQL-CREATETABLE-PARMS-STORAGE)
:   This form sets the storage mode for the column. This controls whether this
    ...
    See [Section 66.2](../../internals/storage/storage-toast.md) for more information.
...
:   The `COMPRESSION` clause sets the compression method
    ...
    is `main` or `extended`.
    (See [ALTER TABLE](sql-altertable.md) for information on
    ...
    *`compression_method`*
    ...
    [default_toast_compression](../../server-administration/runtime-config/runtime-config-client.md#GUC-DEFAULT-TOAST-COMPRESSION) setting at the time of
    data insertion to determine the method to use.
<a id="SQL-CREATETABLE-PARMS-INHERITS"></a>
...
:   The optional `INHERITS` clause specifies a list of
    ...
    column if desired.
<a id="SQL-CREATETABLE-PARMS-PARTITION-BY"></a>
...
:   The optional `PARTITION BY` clause specifies a strategy
    ...
    *partitioned* table. The parenthesized list of
    ...
    must implement support function 2 (see [Section 36.16.3](../../server-programming/extend/xindex.md#XINDEX-SUPPORT)
    for details).
    ...
    See [Section 5.12](../../the-sql-language/ddl/ddl-partitioning.md) for more discussion on table
    partitioning.
<a id="SQL-CREATETABLE-PARTITION"></a>
...
:   Creates the table as a *partition* of the specified
    ...
    *`partition_bound_expr`* is
    ...
    rules of row-wise comparison ([Section 9.25.5](../../the-sql-language/functions/functions-comparisons.md#ROW-WISE-COMPARISON)).
...
:   The `LIKE` clause specifies a table from which
    ...
    <a id="SQL-CREATETABLE-PARMS-LIKE-OPT-COMMENTS"></a>
    ...
    :   Comments for the copied columns, check constraints,
        ...
        comments.
    <a id="SQL-CREATETABLE-PARMS-LIKE-OPT-COMPRESSION"></a>
    ...
    <a id="SQL-CREATETABLE-PARMS-LIKE-OPT-CONSTRAINTS"></a>
    ...
    <a id="SQL-CREATETABLE-PARMS-LIKE-OPT-DEFAULTS"></a>
    ...
    <a id="SQL-CREATETABLE-PARMS-LIKE-OPT-GENERATED"></a>
    ...
    <a id="SQL-CREATETABLE-PARMS-LIKE-OPT-IDENTITY"></a>
    ...
    :   Any identity specifications of copied column definitions will be
        ...
        table.
    <a id="SQL-CREATETABLE-PARMS-LIKE-OPT-INDEXES"></a>
    ...
    <a id="SQL-CREATETABLE-PARMS-LIKE-OPT-STATISTICS"></a>

    `INCLUDING STATISTICS` [#](#SQL-CREATETABLE-PARMS-LIKE-OPT-STATISTICS)
    :   Extended statistics are copied to the new table.
    <a id="SQL-CREATETABLE-PARMS-LIKE-OPT-STORAGE"></a>
    ...
    :   `STORAGE` settings for the copied column
        ...
        `STORAGE` settings, see [Section 66.2](../../internals/storage/storage-toast.md).
    <a id="SQL-CREATETABLE-PARMS-LIKE-OPT-ALL"></a>
    ...
    a view) are ignored.
<a id="SQL-CREATETABLE-PARMS-CONSTRAINT"></a>
...
<a id="SQL-CREATETABLE-PARMS-NOT-NULL"></a>
...
:   The column is not allowed to contain null values.
    ...
    child tables.
<a id="SQL-CREATETABLE-PARMS-NULL"></a>
...
:   The column is allowed to contain null values. This is the default.
    ...
    applications.
<a id="SQL-CREATETABLE-PARMS-CHECK"></a>
...
:   The `CHECK` clause specifies an expression producing a
    ...
    current row (see [Section 5.5.1](../../the-sql-language/ddl/ddl-constraints.md#DDL-CONSTRAINTS-CHECK-CONSTRAINTS)).
...
<a id="SQL-CREATETABLE-PARMS-DEFAULT"></a>
...
<a id="SQL-CREATETABLE-PARMS-GENERATED-STORED"></a>

`GENERATED ALWAYS AS ( generation_expr ) [ STORED | VIRTUAL ]`<a id="id-1.9.3.85.6.2.20.1.2"></a> [#](#SQL-CREATETABLE-PARMS-GENERATED-STORED)
:   This clause creates the column as a *generated
    ...
    `STORED` is specified, the column will be computed on
    write and will be stored on disk. `VIRTUAL` is the
    default.
...
<a id="SQL-CREATETABLE-PARMS-GENERATED-IDENTITY"></a>
...
:   This clause creates the column as an *identity
    ...
    user-specified value takes precedence. See [INSERT](sql-insert.md)
    ...
    options include those shown for [CREATE SEQUENCE](sql-createsequence.md),
...
<a id="SQL-CREATETABLE-PARMS-UNIQUE"></a>

`UNIQUE [ NULLS [ NOT ] DISTINCT ]` (column constraint)<br>`UNIQUE [ NULLS [ NOT ] DISTINCT ] ( column_name [, ... ] [, column_name WITHOUT OVERLAPS ] )` [ `INCLUDE ( column_name [, ...])` ] (table constraint) [#](#SQL-CREATETABLE-PARMS-UNIQUE)
:   The `UNIQUE` constraint specifies that a
    ...
    If the `WITHOUT OVERLAPS` option is specified for the
    ...
    `WITHOUT OVERLAPS` column. (This is sometimes called a
    temporal key, if the column is a range of dates or timestamps, but
    PostgreSQL allows ranges over any base type.)
    In effect, such a constraint is enforced with an `EXCLUDE`
    constraint rather than a `UNIQUE` constraint. So for
    example `UNIQUE (id, valid_at WITHOUT OVERLAPS)` behaves
    like `EXCLUDE USING GIST (id WITH =, valid_at WITH
    &&)`. The `WITHOUT OVERLAPS` column
    must have a range or multirange type. Empty ranges/multiranges are
    not permitted. The non-`WITHOUT OVERLAPS` columns of
    ...
    other types by adding the [btree_gist](../../appendixes/contrib/btree-gist.md) extension (which
    ...
    index deletion.
<a id="SQL-CREATETABLE-PARMS-PRIMARY-KEY"></a>

`PRIMARY KEY` (column constraint)<br>`PRIMARY KEY ( column_name [, ... ] [, column_name WITHOUT OVERLAPS ] )` [ `INCLUDE ( column_name [, ...])` ] (table constraint) [#](#SQL-CREATETABLE-PARMS-PRIMARY-KEY)
:   The `PRIMARY KEY` constraint specifies that a column or
    ...
    the constraint, or GiST if `WITHOUT OVERLAPS` was
    specified.
    ...
    (e.g., `DROP COLUMN`) can cause cascaded constraint and
    index deletion.
<a id="SQL-CREATETABLE-EXCLUDE"></a>
...
:   The `EXCLUDE` clause defines an exclusion
    ...
    (see [Section 8.8](../../the-sql-language/datatype/datatype-geometric.md)) by using the
    `&&` operator.
    ...
    (see [Section 11.10](../../the-sql-language/indexes/indexes-opclass.md)) for the index access
    method *`index_method`*.
    ...
    these are described fully under [CREATE INDEX](sql-createindex.md).

    The access method must support `amgettuple` (see [Chapter 63](../../internals/indexam/README.md)); at present this means GIN
...
<a id="SQL-CREATETABLE-PARMS-REFERENCES"></a>

`REFERENCES reftable [ ( refcolumn ) ] [ MATCH matchtype ] [ ON DELETE referential_action ] [ ON UPDATE referential_action ]` (column constraint)<br>`FOREIGN KEY ( column_name [, ... ] [, PERIOD column_name ] ) REFERENCES reftable [ ( refcolumn [, ... ] [, PERIOD refcolumn ] ) ] [ MATCH matchtype ] [ ON DELETE referential_action ] [ ON UPDATE referential_action ]` (table constraint) [#](#SQL-CREATETABLE-PARMS-REFERENCES)
:   These clauses specify a foreign key constraint, which requires
    ...
    *`refcolumn`* clause is omitted,
    ...
    <a id="SQL-CREATETABLE-PARMS-REFERENCES-REFACT-NO-ACTION"></a>
    ...
    <a id="SQL-CREATETABLE-PARMS-REFERENCES-REFACT-RESTRICT"></a>
    ...
    :   Produce an error if a row to be deleted or updated matches a row in
        ...
        “no-op” updates that update a column to the same
        value.)

        In a temporal foreign key, this option is not supported.
    <a id="SQL-CREATETABLE-PARMS-REFERENCES-REFACT-CASCADE"></a>
    ...
    :   Delete any rows referencing the deleted row, or update the
        ...
        In a temporal foreign key, this option is not supported.
    <a id="SQL-CREATETABLE-PARMS-REFERENCES-REFACT-SET-NULL"></a>
    ...
    :   Set all of the referencing columns, or a specified subset of the
        ...
        In a temporal foreign key, this option is not supported.
    <a id="SQL-CREATETABLE-PARMS-REFERENCES-REFACT-SET-DEFAULT"></a>
    ...
    efficiently.
<a id="SQL-CREATETABLE-PARMS-DEFERRABLE"></a>

`DEFERRABLE`<br>`NOT DEFERRABLE` [#](#SQL-CREATETABLE-PARMS-DEFERRABLE)
:   This controls whether the constraint can be deferred. A
    constraint that is not deferrable will be checked immediately
    after every command. Checking of constraints that are
    deferrable can be postponed until the end of the transaction
    (using the [`SET CONSTRAINTS`](sql-set-constraints.md) command).
...
<a id="SQL-CREATETABLE-PARMS-INITIALLY"></a>

`INITIALLY IMMEDIATE`<br>`INITIALLY DEFERRED` [#](#SQL-CREATETABLE-PARMS-INITIALLY)
:   If a constraint is deferrable, this clause specifies the default
    ...
    altered with the [`SET CONSTRAINTS`](sql-set-constraints.md) command.
<a id="SQL-CREATETABLE-PARMS-ENFORCED"></a>

`ENFORCED`<br>`NOT ENFORCED` [#](#SQL-CREATETABLE-PARMS-ENFORCED)
:   When the constraint is `ENFORCED`, then the database
    ...
    `NOT ENFORCED` constraints can be useful as
    ...
    constraints.
<a id="SQL-CREATETABLE-METHOD"></a>

`USING method` [#](#SQL-CREATETABLE-METHOD)
:   This optional clause specifies the table access method to use to store
    the contents for the new table; the method needs be an access method of
    type `TABLE`. See [Chapter 62](../../internals/tableam/README.md) for more
    information. If this option is not specified, the default table access
    method is chosen for the new table. See [default_table_access_method](../../server-administration/runtime-config/runtime-config-client.md#GUC-DEFAULT-TABLE-ACCESS-METHOD) for more information.

    When creating a partition, the table access method is the access method
    of its partitioned table, if set.
<a id="SQL-CREATETABLE-PARMS-WITH"></a>

`WITH ( storage_parameter [= value] [, ... ] )` [#](#SQL-CREATETABLE-PARMS-WITH)
:   This clause specifies optional storage parameters for a table or index;
    see [Storage Parameters](sql-createtable.md#SQL-CREATETABLE-STORAGE-PARAMETERS) below for more
...
<a id="SQL-CREATETABLE-PARMS-WITHOUT-OIDS"></a>
...
<a id="SQL-CREATETABLE-PARMS-ON-COMMIT"></a>
...
:   The behavior of temporary tables at the end of a transaction
    ...
    <a id="SQL-CREATETABLE-PARMS-ON-COMMIT-PRESERVE-ROWS"></a>

    `PRESERVE ROWS` [#](#SQL-CREATETABLE-PARMS-ON-COMMIT-PRESERVE-ROWS)
    :   No special action is taken at the ends of transactions.
        This is the default behavior.
    <a id="SQL-CREATETABLE-PARMS-ON-COMMIT-DELETE-ROWS"></a>

    `DELETE ROWS` [#](#SQL-CREATETABLE-PARMS-ON-COMMIT-DELETE-ROWS)
    :   All rows in the temporary table will be deleted at the end
        of each transaction block. Essentially, an automatic [`TRUNCATE`](sql-truncate.md) is done
        at each commit. When used on a partitioned table, this
        is not cascaded to its partitions.
    <a id="SQL-CREATETABLE-PARMS-ON-COMMIT-DROP"></a>
...
<a id="SQL-CREATETABLE-TABLESPACE"></a>

`TABLESPACE tablespace_name` [#](#SQL-CREATETABLE-TABLESPACE)
:   The *`tablespace_name`* is the name
    of the tablespace in which the new table is to be created.
    If not specified,
    [default_tablespace](../../server-administration/runtime-config/runtime-config-client.md#GUC-DEFAULT-TABLESPACE) is consulted, or
    [temp_tablespaces](../../server-administration/runtime-config/runtime-config-client.md#GUC-TEMP-TABLESPACES) if the table is temporary. For
...
<a id="SQL-CREATETABLE-PARMS-USING-INDEX-TABLESPACE"></a>
...
:   This clause allows selection of the tablespace in which the index
    ...
    If not specified,
    [default_tablespace](../../server-administration/runtime-config/runtime-config-client.md#GUC-DEFAULT-TABLESPACE) is consulted, or
    [temp_tablespaces](../../server-administration/runtime-config/runtime-config-client.md#GUC-TEMP-TABLESPACES) if the table is temporary.

<a id="SQL-CREATETABLE-STORAGE-PARAMETERS"></a>

### Storage Parameters

<a id="id-1.9.3.85.6.3.2"></a>
...
indexes are documented in [CREATE INDEX](sql-createindex.md).
The storage parameters currently
available for tables are listed below. For many of these parameters, as
shown, there is an additional parameter with the same name prefixed with
`toast.`, which controls the behavior of the
table's secondary TOAST table, if any
(see [Section 66.2](../../internals/storage/storage-toast.md) for more information about TOAST).
...
<a id="RELOPTION-FILLFACTOR"></a>

`fillfactor` (`integer`) <a id="id-1.9.3.85.6.3.4.1.1.3"></a> [#](#RELOPTION-FILLFACTOR)
:   The fillfactor for a table is a percentage between 10 and 100.
    ...
    updates](../../internals/storage/storage-hot.md) more likely.
    For a table whose entries are never updated, complete packing is the
    best choice, but in heavily updated tables smaller fillfactors are
    appropriate. This parameter cannot be set for TOAST tables.
<a id="RELOPTION-TOAST-TUPLE-TARGET"></a>

`toast_tuple_target` (`integer`) <a id="id-1.9.3.85.6.3.4.2.1.3"></a> [#](#RELOPTION-TOAST-TUPLE-TARGET)
...
<a id="RELOPTION-PARALLEL-WORKERS"></a>

`parallel_workers` (`integer`) <a id="id-1.9.3.85.6.3.4.3.1.3"></a> [#](#RELOPTION-PARALLEL-WORKERS)
:   This sets the number of workers that should be used to assist a parallel
    scan of this table. If not set, the system will determine a value based
    on the relation size. The actual number of workers chosen by the planner
    or by utility statements that use parallel scans may be less, for example
    due to the setting of [max_worker_processes](../../server-administration/runtime-config/runtime-config-resource.md#GUC-MAX-WORKER-PROCESSES).
<a id="RELOPTION-AUTOVACUUM-ENABLED"></a>

`autovacuum_enabled`, `toast.autovacuum_enabled` (`boolean`) <a id="id-1.9.3.85.6.3.4.4.1.4"></a> [#](#RELOPTION-AUTOVACUUM-ENABLED)
:   Enables or disables the autovacuum daemon for a particular table.
    ...
    discussed in [Section 24.1.6](../../server-administration/maintenance/routine-vacuuming.md#AUTOVACUUM).
    If false, this table will not be autovacuumed, except to prevent
    transaction ID wraparound. See [Section 24.1.5](../../server-administration/maintenance/routine-vacuuming.md#VACUUM-FOR-WRAPAROUND) for
    more about wraparound prevention.
    Note that the autovacuum daemon does not run at all (except to prevent
    transaction ID wraparound) if the [autovacuum](../../server-administration/runtime-config/runtime-config-vacuum.md#GUC-AUTOVACUUM)
    ...
    to `false`.
<a id="RELOPTION-VACUUM-INDEX-CLEANUP"></a>

`vacuum_index_cleanup`, `toast.vacuum_index_cleanup` (`enum`) <a id="id-1.9.3.85.6.3.4.5.1.4"></a> [#](#RELOPTION-VACUUM-INDEX-CLEANUP)
:   Forces or disables index cleanup when `VACUUM`
    ...
    `INDEX_CLEANUP` parameter of [`VACUUM`](sql-vacuum.md), if
    specified, overrides the value of this option.
<a id="RELOPTION-VACUUM-TRUNCATE"></a>

`vacuum_truncate`, `toast.vacuum_truncate` (`boolean`) <a id="id-1.9.3.85.6.3.4.6.1.4"></a> [#](#RELOPTION-VACUUM-TRUNCATE)
:   Per-table value for [vacuum_truncate](../../server-administration/runtime-config/runtime-config-vacuum.md#GUC-VACUUM-TRUNCATE) parameter. The
    `TRUNCATE` parameter of
    [`VACUUM`](sql-vacuum.md), if
    specified, overrides the value of this option.
<a id="RELOPTION-AUTOVACUUM-VACUUM-THRESHOLD"></a>

`autovacuum_vacuum_threshold`, `toast.autovacuum_vacuum_threshold` (`integer`) <a id="id-1.9.3.85.6.3.4.7.1.4"></a> [#](#RELOPTION-AUTOVACUUM-VACUUM-THRESHOLD)
:   Per-table value for [autovacuum_vacuum_threshold](../../server-administration/runtime-config/runtime-config-vacuum.md#GUC-AUTOVACUUM-VACUUM-THRESHOLD)
    parameter.
<a id="RELOPTION-AUTOVACUUM-VACUUM-MAX-THRESHOLD"></a>

`autovacuum_vacuum_max_threshold`, `toast.autovacuum_vacuum_max_threshold` (`integer`) <a id="id-1.9.3.85.6.3.4.8.1.4"></a> [#](#RELOPTION-AUTOVACUUM-VACUUM-MAX-THRESHOLD)
:   Per-table value for [autovacuum_vacuum_max_threshold](../../server-administration/runtime-config/runtime-config-vacuum.md#GUC-AUTOVACUUM-VACUUM-MAX-THRESHOLD)
    parameter.
<a id="RELOPTION-AUTOVACUUM-VACUUM-SCALE-FACTOR"></a>

`autovacuum_vacuum_scale_factor`, `toast.autovacuum_vacuum_scale_factor` (`floating point`) <a id="id-1.9.3.85.6.3.4.9.1.4"></a> [#](#RELOPTION-AUTOVACUUM-VACUUM-SCALE-FACTOR)
:   Per-table value for [autovacuum_vacuum_scale_factor](../../server-administration/runtime-config/runtime-config-vacuum.md#GUC-AUTOVACUUM-VACUUM-SCALE-FACTOR)
    parameter.
<a id="RELOPTION-AUTOVACUUM-VACUUM-INSERT-THRESHOLD"></a>

`autovacuum_vacuum_insert_threshold`, `toast.autovacuum_vacuum_insert_threshold` (`integer`) <a id="id-1.9.3.85.6.3.4.10.1.4"></a> [#](#RELOPTION-AUTOVACUUM-VACUUM-INSERT-THRESHOLD)
:   Per-table value for [autovacuum_vacuum_insert_threshold](../../server-administration/runtime-config/runtime-config-vacuum.md#GUC-AUTOVACUUM-VACUUM-INSERT-THRESHOLD)
    parameter. The special value of -1 may be used to disable insert vacuums on the table.
<a id="RELOPTION-AUTOVACUUM-VACUUM-INSERT-SCALE-FACTOR"></a>

`autovacuum_vacuum_insert_scale_factor`, `toast.autovacuum_vacuum_insert_scale_factor` (`floating point`) <a id="id-1.9.3.85.6.3.4.11.1.4"></a> [#](#RELOPTION-AUTOVACUUM-VACUUM-INSERT-SCALE-FACTOR)
:   Per-table value for [autovacuum_vacuum_insert_scale_factor](../../server-administration/runtime-config/runtime-config-vacuum.md#GUC-AUTOVACUUM-VACUUM-INSERT-SCALE-FACTOR)
    parameter.
<a id="RELOPTION-AUTOVACUUM-ANALYZE-THRESHOLD"></a>

`autovacuum_analyze_threshold` (`integer`) <a id="id-1.9.3.85.6.3.4.12.1.3"></a> [#](#RELOPTION-AUTOVACUUM-ANALYZE-THRESHOLD)
:   Per-table value for [autovacuum_analyze_threshold](../../server-administration/runtime-config/runtime-config-vacuum.md#GUC-AUTOVACUUM-ANALYZE-THRESHOLD)
    parameter.
<a id="RELOPTION-AUTOVACUUM-ANALYZE-SCALE-FACTOR"></a>

`autovacuum_analyze_scale_factor` (`floating point`) <a id="id-1.9.3.85.6.3.4.13.1.3"></a> [#](#RELOPTION-AUTOVACUUM-ANALYZE-SCALE-FACTOR)
:   Per-table value for [autovacuum_analyze_scale_factor](../../server-administration/runtime-config/runtime-config-vacuum.md#GUC-AUTOVACUUM-ANALYZE-SCALE-FACTOR)
    parameter.
<a id="RELOPTION-AUTOVACUUM-VACUUM-COST-DELAY"></a>

`autovacuum_vacuum_cost_delay`, `toast.autovacuum_vacuum_cost_delay` (`floating point`) <a id="id-1.9.3.85.6.3.4.14.1.4"></a> [#](#RELOPTION-AUTOVACUUM-VACUUM-COST-DELAY)
:   Per-table value for [autovacuum_vacuum_cost_delay](../../server-administration/runtime-config/runtime-config-vacuum.md#GUC-AUTOVACUUM-VACUUM-COST-DELAY)
    parameter.
<a id="RELOPTION-AUTOVACUUM-VACUUM-COST-LIMIT"></a>

`autovacuum_vacuum_cost_limit`, `toast.autovacuum_vacuum_cost_limit` (`integer`) <a id="id-1.9.3.85.6.3.4.15.1.4"></a> [#](#RELOPTION-AUTOVACUUM-VACUUM-COST-LIMIT)
:   Per-table value for [autovacuum_vacuum_cost_limit](../../server-administration/runtime-config/runtime-config-vacuum.md#GUC-AUTOVACUUM-VACUUM-COST-LIMIT)
    parameter.
<a id="RELOPTION-AUTOVACUUM-FREEZE-MIN-AGE"></a>

`autovacuum_freeze_min_age`, `toast.autovacuum_freeze_min_age` (`integer`) <a id="id-1.9.3.85.6.3.4.16.1.4"></a> [#](#RELOPTION-AUTOVACUUM-FREEZE-MIN-AGE)
:   Per-table value for [vacuum_freeze_min_age](../../server-administration/runtime-config/runtime-config-vacuum.md#GUC-VACUUM-FREEZE-MIN-AGE)
    parameter. Note that autovacuum will ignore
    per-table `autovacuum_freeze_min_age` parameters that are
    larger than half the
    system-wide [autovacuum_freeze_max_age](../../server-administration/runtime-config/runtime-config-vacuum.md#GUC-AUTOVACUUM-FREEZE-MAX-AGE) setting.
<a id="RELOPTION-AUTOVACUUM-FREEZE-MAX-AGE"></a>

`autovacuum_freeze_max_age`, `toast.autovacuum_freeze_max_age` (`integer`) <a id="id-1.9.3.85.6.3.4.17.1.4"></a> [#](#RELOPTION-AUTOVACUUM-FREEZE-MAX-AGE)
:   Per-table value for [autovacuum_freeze_max_age](../../server-administration/runtime-config/runtime-config-vacuum.md#GUC-AUTOVACUUM-FREEZE-MAX-AGE)
    parameter. Note that autovacuum will ignore
    per-table `autovacuum_freeze_max_age` parameters that are
    larger than the system-wide setting (it can only be set smaller).
<a id="RELOPTION-AUTOVACUUM-FREEZE-TABLE-AGE"></a>

`autovacuum_freeze_table_age`, `toast.autovacuum_freeze_table_age` (`integer`) <a id="id-1.9.3.85.6.3.4.18.1.4"></a> [#](#RELOPTION-AUTOVACUUM-FREEZE-TABLE-AGE)
:   Per-table value for [vacuum_freeze_table_age](../../server-administration/runtime-config/runtime-config-vacuum.md#GUC-VACUUM-FREEZE-TABLE-AGE)
    parameter.
<a id="RELOPTION-AUTOVACUUM-MULTIXACT-FREEZE-MIN-AGE"></a>

`autovacuum_multixact_freeze_min_age`, `toast.autovacuum_multixact_freeze_min_age` (`integer`) <a id="id-1.9.3.85.6.3.4.19.1.4"></a> [#](#RELOPTION-AUTOVACUUM-MULTIXACT-FREEZE-MIN-AGE)
:   Per-table value for [vacuum_multixact_freeze_min_age](../../server-administration/runtime-config/runtime-config-vacuum.md#GUC-VACUUM-MULTIXACT-FREEZE-MIN-AGE)
    parameter. Note that autovacuum will ignore
    per-table `autovacuum_multixact_freeze_min_age` parameters
    that are larger than half the
    system-wide [autovacuum_multixact_freeze_max_age](../../server-administration/runtime-config/runtime-config-vacuum.md#GUC-AUTOVACUUM-MULTIXACT-FREEZE-MAX-AGE)
    setting.
<a id="RELOPTION-AUTOVACUUM-MULTIXACT-FREEZE-MAX-AGE"></a>

`autovacuum_multixact_freeze_max_age`, `toast.autovacuum_multixact_freeze_max_age` (`integer`) <a id="id-1.9.3.85.6.3.4.20.1.4"></a> [#](#RELOPTION-AUTOVACUUM-MULTIXACT-FREEZE-MAX-AGE)
:   Per-table value
    for [autovacuum_multixact_freeze_max_age](../../server-administration/runtime-config/runtime-config-vacuum.md#GUC-AUTOVACUUM-MULTIXACT-FREEZE-MAX-AGE) parameter.
    ...
    smaller).
<a id="RELOPTION-AUTOVACUUM-MULTIXACT-FREEZE-TABLE-AGE"></a>

`autovacuum_multixact_freeze_table_age`, `toast.autovacuum_multixact_freeze_table_age` (`integer`) <a id="id-1.9.3.85.6.3.4.21.1.4"></a> [#](#RELOPTION-AUTOVACUUM-MULTIXACT-FREEZE-TABLE-AGE)
:   Per-table value
    for [vacuum_multixact_freeze_table_age](../../server-administration/runtime-config/runtime-config-vacuum.md#GUC-VACUUM-MULTIXACT-FREEZE-TABLE-AGE) parameter.
<a id="RELOPTION-LOG-AUTOVACUUM-MIN-DURATION"></a>

`log_autovacuum_min_duration`, `toast.log_autovacuum_min_duration` (`integer`) <a id="id-1.9.3.85.6.3.4.22.1.4"></a> [#](#RELOPTION-LOG-AUTOVACUUM-MIN-DURATION)
:   Per-table value for [log_autovacuum_min_duration](../../server-administration/runtime-config/runtime-config-logging.md#GUC-LOG-AUTOVACUUM-MIN-DURATION)
    parameter.
<a id="RELOPTION-VACUUM-MAX-EAGER-FREEZE-FAILURE-RATE"></a>

`vacuum_max_eager_freeze_failure_rate`, `toast.vacuum_max_eager_freeze_failure_rate` (`floating point`) <a id="id-1.9.3.85.6.3.4.23.1.4"></a> [#](#RELOPTION-VACUUM-MAX-EAGER-FREEZE-FAILURE-RATE)
:   Per-table value for [vacuum_max_eager_freeze_failure_rate](../../server-administration/runtime-config/runtime-config-vacuum.md#GUC-VACUUM-MAX-EAGER-FREEZE-FAILURE-RATE)
    parameter.
<a id="RELOPTION-USER-CATALOG-TABLE"></a>

`user_catalog_table` (`boolean`) <a id="id-1.9.3.85.6.3.4.24.1.3"></a> [#](#RELOPTION-USER-CATALOG-TABLE)
:   Declare the table as an additional catalog table for purposes of
    logical replication. See
    [Section 47.6.2](../../server-programming/logicaldecoding/logicaldecoding-output-plugin.md#LOGICALDECODING-CAPABILITIES) for details.
    This parameter cannot be set for TOAST tables.
...
index explicitly for primary key columns. (See [CREATE INDEX](sql-createindex.md) for more information.)
...
<a id="id-1.9.3.85.9.13"></a>

### Generated Columns

The options `STORED` and `VIRTUAL` are
...
<a id="id-1.9.3.85.10"></a>

## See Also

[ALTER TABLE](sql-altertable.md), [DROP TABLE](sql-droptable.md), [CREATE TABLE AS](sql-createtableas.md), [CREATE TABLESPACE](sql-createtablespace.md), [CREATE TYPE](sql-createtype.md)

---

原文：[PostgreSQL 18.6 Documentation](https://www.postgresql.org/docs/18/sql-createtable.html)（英文原文，待翻譯）
