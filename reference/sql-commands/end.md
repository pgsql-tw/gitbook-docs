...
`END` commits the current transaction. All changes made by the transaction become visible to others and are guaranteed to be durable if a crash occurs. This command is a PostgreSQL extension that is equivalent to [`COMMIT`](commit.md).
...
`END` is a PostgreSQL extension that provides functionality equivalent to [`COMMIT`](commit.md), which is specified in the SQL standard.
...
原文：[PostgreSQL 15.19 Documentation](https://www.postgresql.org/docs/15/sql-end.html)（英文原文，待翻譯）
