...
`EXECUTE` statement. Refer to [EXECUTE](sql-execute.md) for more
...
manually cleaned up using the [`DEALLOCATE`](sql-deallocate.md) command.
...
By default (that is, when [plan_cache_mode](../../server-administration/runtime-config/runtime-config-query.md#GUC-PLAN-CACHE-MODE) is set
...
for a prepared statement, use [`EXPLAIN`](sql-explain.md), for example
...
[`pg_prepared_statements`](../../internals/views/view-pg-prepared-statements.md)
system view.
...
原文：[PostgreSQL 18.6 Documentation](https://www.postgresql.org/docs/18/sql-prepare.html)（英文原文，待翻譯）
