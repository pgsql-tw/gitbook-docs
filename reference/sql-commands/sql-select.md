<a id="id-1.9.3.172.1"></a><a id="id-1.9.3.172.2"></a><a id="id-1.9.3.172.3"></a>
...
1. All queries in the `WITH` list are computed.
   ...
   (See [WITH Clause](sql-select.md#SQL-WITH) below.)
2. All elements in the `FROM` list are computed.
   ...
   (See [FROM Clause](sql-select.md#SQL-FROM) below.)
3. If the `WHERE` clause is specified, all rows
   that do not satisfy the condition are eliminated from the
   output. (See [WHERE Clause](sql-select.md#SQL-WHERE) below.)
4. If the `GROUP BY` clause is specified,
   ...
   [GROUP BY Clause](sql-select.md#SQL-GROUPBY) and
   [HAVING Clause](sql-select.md#SQL-HAVING) below.)
...
5. The actual output rows are computed using the
   ...
   row or row group. (See [SELECT List](sql-select.md#SQL-SELECT-LIST) below.)
6. `SELECT DISTINCT` eliminates duplicate rows from the
   result. `SELECT DISTINCT ON` eliminates rows that
   match on all the specified expressions. `SELECT ALL`
   (the default) will return all candidate rows, including
   duplicates. (See [DISTINCT Clause](sql-select.md#SQL-DISTINCT) below.)
7. Using the operators `UNION`,
   ...
   [UNION Clause](sql-select.md#SQL-UNION), [INTERSECT Clause](sql-select.md#SQL-INTERSECT), and
   [EXCEPT Clause](sql-select.md#SQL-EXCEPT) below.)
8. If the `ORDER BY` clause is specified, the
   returned rows are sorted in the specified order. If
   `ORDER BY` is not given, the rows are returned
   in whatever order the system finds fastest to produce. (See
   [ORDER BY Clause](sql-select.md#SQL-ORDERBY) below.)
9. If the `LIMIT` (or `FETCH FIRST`) or `OFFSET`
   clause is specified, the `SELECT` statement
   only returns a subset of the result rows. (See [LIMIT Clause](sql-select.md#SQL-LIMIT) below.)
10. If `FOR UPDATE`, `FOR NO KEY UPDATE`, `FOR SHARE`
    or `FOR KEY SHARE`
    is specified, the
    `SELECT` statement locks the selected rows
    against concurrent updates. (See [The Locking Clause](sql-select.md#SQL-FOR-UPDATE-SHARE)
    below.)
...
`SELECT` query in
a data-modifying statement. See [Section 7.8](../../the-sql-language/queries/queries-with.md) for
an example.
...
[Section 7.8.2.1](../../the-sql-language/queries/queries-with.md#QUERIES-WITH-SEARCH) for examples.
...
used internally for tracking visited rows. See [Section 7.8.2.2](../../the-sql-language/queries/queries-with.md#QUERIES-WITH-CYCLE) for examples.
...
equivalent) commands (no nested `UNION`s). If both
...
See [Section 7.8](../../the-sql-language/queries/queries-with.md) for additional information.
...
:   One of
    ...
    * `FULL [ OUTER ] JOIN`
    ...
    In the absence of parentheses, `JOIN`s nest
...
of grouping sets see [Section 7.2.4](../../the-sql-language/queries/queries-table-expressions.md#QUERIES-GROUPING-SETS).
...
call; see [Section 4.2.7](../../the-sql-language/sql-syntax/sql-expressions.md#SYNTAX-AGGREGATES) for more information. When
...
an aggregate function; see [Section 4.2.14](../../the-sql-language/sql-syntax/sql-expressions.md#SYNTAX-EXPRESS-EVAL).
...
much the same fashion as elements of a [`GROUP BY`](sql-select.md#SQL-GROUPBY) clause, except that
...
in much the same fashion as elements of a statement-level [`ORDER BY`](sql-select.md#SQL-ORDERBY) clause, except that
...
[`SELECT` list](sql-select.md#SQL-SELECT-LIST) or
[`ORDER BY`](sql-select.md#SQL-ORDERBY) clause.
These functions
...
[Section 4.2.8](../../the-sql-language/sql-syntax/sql-expressions.md#SYNTAX-WINDOW-FUNCTIONS), and
[Section 7.2.5](../../the-sql-language/queries/queries-table-expressions.md#QUERIES-WINDOW).
...
PostgreSQL keyword (see [Appendix C](../../appendixes/sql-keywords-appendix/README.md)). For protection against possible
...
For more information see [Section 4.2.10](../../the-sql-language/sql-syntax/sql-expressions.md#SQL-SYNTAX-COLLATE-EXPRS) and
...
row-level lock mode, refer to [Section 13.3.2](../../the-sql-language/mvcc/explicit-locking.md#LOCKING-ROWS).
...
[Chapter 13](../../the-sql-language/mvcc/README.md)). You can use
[`LOCK`](sql-lock.md)
...
else the query will loop indefinitely. (See [Section 7.8](../../the-sql-language/queries/queries-with.md)
for more examples.)
...
in [LIMIT Clause](sql-select.md#SQL-LIMIT). This
...
原文：[PostgreSQL 18.6 Documentation](https://www.postgresql.org/docs/18/sql-select.html)（英文原文，待翻譯）
