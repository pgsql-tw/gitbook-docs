<a id="SQL-LOCK"></a><a id="id-1.9.3.155.1"></a>
...
More information about the lock modes and locking strategies can be found in [Section 13.3](../../the-sql-language/concurrency-control/explicit-locking.md).
...
The lock mode specifies which locks this lock conflicts with. Lock modes are described in [Section 13.3](../../the-sql-language/concurrency-control/explicit-locking.md).
...
There is no `LOCK TABLE` in the SQL standard, which instead uses `SET TRANSACTION` to specify concurrency levels on transactions. PostgreSQL supports that too; see [SET TRANSACTION](set-transaction.md) for details.
