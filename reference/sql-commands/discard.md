...
Discards all cached sequence-related state, including `currval()`/`lastval()` information and any preallocated sequence values that have not yet been returned by `nextval()`. (See [CREATE SEQUENCE](create-sequence.md) for a description of preallocated sequence values.)
...
原文：[PostgreSQL 15.19 Documentation](https://www.postgresql.org/docs/15/sql-discard.html)（英文原文，待翻譯）
