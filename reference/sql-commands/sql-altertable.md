...
:   This form adds a new column to the table, using the same syntax as
    [`CREATE TABLE`](sql-createtable.md). If `IF NOT EXISTS`
...
:   This form changes the type of a column of a table. Indexes and
    ...
    so running [`ANALYZE`](sql-analyze.md)
...
`SET EXPRESSION AS` [#](#SQL-ALTERTABLE-DESC-SET-EXPRESSION)
...
`ADD GENERATED { ALWAYS | BY DEFAULT } AS IDENTITY`<br>`SET GENERATED { ALWAYS | BY DEFAULT }`<br>`DROP IDENTITY [ IF EXISTS ]` [#](#SQL-ALTERTABLE-DESC-GENERATED-IDENTITY)
:   These forms change whether a column is an identity column or change the
    ...
    See [`CREATE TABLE`](sql-createtable.md) for details.
...
`SET sequence_option`<br>`RESTART` [#](#SQL-ALTERTABLE-DESC-SET-SEQUENCE-OPTION)
:   These forms alter the sequence that underlies an existing identity
    column. *`sequence_option`* is an option
    supported by [`ALTER SEQUENCE`](sql-altersequence.md) such
    as `INCREMENT BY`.
...
`SET ( attribute_option = value [, ... ] )`<br>`RESET ( attribute_option [, ... ] )` [#](#SQL-ALTERTABLE-DESC-SET-ATTRIBUTE-OPTION)
...
`SET STORAGE { PLAIN | EXTERNAL | EXTENDED | MAIN | DEFAULT }` <a id="id-1.9.3.35.5.2.3.12.1.2"></a> [#](#SQL-ALTERTABLE-DESC-SET-STORAGE)
...
:   This form sets the compression method for a column, determining how
    ...
    consulting the [default_toast_compression](../../server-administration/runtime-config/runtime-config-client.md#GUC-DEFAULT-TOAST-COMPRESSION) setting
...
:   This form adds a new constraint to a table using the same constraint
    syntax as [`CREATE TABLE`](sql-createtable.md), plus the option `NOT
    ...
    CONSTRAINT` option.
    See [Notes](sql-altertable.md#SQL-ALTERTABLE-NOTES) below for more information
    ...
    are added to partitioned tables; see [`CREATE TABLE`](sql-createtable.md).
...
:   This form adds a new `PRIMARY KEY` or `UNIQUE`
    ...
    `CREATE UNIQUE INDEX CONCURRENTLY`, and then convert it to a
...
`ALTER CONSTRAINT ... INHERIT`<br>`ALTER CONSTRAINT ... NO INHERIT` [#](#SQL-ALTERTABLE-DESC-ALTER-CONSTRAINT-INHERIT)
...
:   This form validates a foreign key, check, or not-null constraint that was
    ...
    (See [Notes](sql-altertable.md#SQL-ALTERTABLE-NOTES) below for an explanation
...
:   These forms configure the firing of trigger(s) belonging to the table.
    ...
    variable [session_replication_role](../../server-administration/runtime-config/runtime-config-client.md#GUC-SESSION-REPLICATION-ROLE). Simply enabled
...
:   These forms configure the firing of rewrite rules belonging to the table.
    ...
    [session_replication_role](../../server-administration/runtime-config/runtime-config-client.md#GUC-SESSION-REPLICATION-ROLE), analogous to triggers as
    described above.
...
:   This form changes the access method of the table by rewriting it
    ...
    [default_table_access_method](../../server-administration/runtime-config/runtime-config-client.md#GUC-DEFAULT-TABLE-ACCESS-METHOD) configuration
    parameter.
    See [Chapter 62](../../internals/tableam/README.md) for more information.
...
:   This form changes the table from unlogged to logged or vice-versa
    (see [`UNLOGGED`](sql-createtable.md#SQL-CREATETABLE-UNLOGGED)). It cannot be applied
...
:   This form changes one or more storage parameters for the table. See
    ...
    [`CREATE TABLE`](sql-createtable.md) documentation
    for details on the available parameters. Note that the table contents
    will not be modified immediately by this command; depending on the
    parameter you might need to rewrite the table to get the desired effects.
    That can be done with [`VACUUM
    FULL`](sql-vacuum.md), [`CLUSTER`](sql-cluster.md) or one of the forms
...
:   This form changes the information which is written to the write-ahead log
    ...
    :   Records the old values of the columns of the primary key.
        ...
        When there is no primary key, the behavior is the same as `NOTHING`.
...
:   This form attaches an existing table (which might itself be partitioned)
    ...
    [CREATE FOREIGN TABLE](sql-createforeigntable.md).) For each user-defined
    ...
    *`partition_bound_spec`* as
    ...
    (See the discussion in [CREATE FOREIGN TABLE](sql-createforeigntable.md) about
    ...
    [Section 5.12.2.2](../../the-sql-language/ddl/ddl-partitioning.md#DDL-PARTITIONING-DECLARATIVE-MAINTENANCE).
...
:   The partition bound specification for a new partition. Refer to
    [CREATE TABLE](sql-createtable.md) for more details on the syntax of the same.
...
non-volatile `DEFAULT` is specified, the default value is
...
`DEFAULT`. In neither case is a rewrite of the table
required.
...
occurred. See [Section 13.6](../../the-sql-language/mvcc/mvcc-caveats.md) for more details.
...
Refer to [CREATE TABLE](sql-createtable.md) for a further description of valid
parameters. [Chapter 5](../../the-sql-language/ddl/README.md) has further information on
inheritance.

<a id="id-1.9.3.35.8"></a>
...
原文：[PostgreSQL 18.6 Documentation](https://www.postgresql.org/docs/18/sql-altertable.html)（英文原文，待翻譯）
