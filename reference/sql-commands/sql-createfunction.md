...
Refer to [Section 36.3](../../server-programming/extend/xfunc.md) for further information on writing
functions.

<a id="id-1.9.3.67.6"></a>
...
:   The name of an argument. Some languages (including SQL and PL/pgSQL)
    ...
    when calling a function to improve readability (see [Section 4.3](../../the-sql-language/sql-syntax/sql-syntax-calling-funcs.md)). In any case, the name
...
:   Lists which transforms a call to the function should apply. Transforms
    ...
    see [CREATE TRANSFORM](sql-createtransform.md). Procedural language
...
:   These attributes inform the query optimizer about the behavior
    ...
    For additional details see [Section 36.7](../../server-programming/extend/xfunc-volatility.md).
...
:   `LEAKPROOF` indicates that the function has no side
    ...
    [CREATE VIEW](sql-createview.md) and [Section 39.5](../../server-programming/rules/rules-privileges.md).
...
:   `SECURITY INVOKER` indicates that the function
    ...
    [see below](sql-createfunction.md#SQL-CREATEFUNCTION-SECURITY).
...
:   A positive number giving the estimated execution cost for the function,
    in units of [cpu_operator_cost](../../server-administration/runtime-config/runtime-config-query.md#GUC-CPU-OPERATOR-COST). If the function
...
:   The name (optionally schema-qualified) of a *planner support
    ...
    [Section 36.11](../../server-programming/extend/xfunc-optimization.md) for details.
...
:   The `SET` clause causes the specified configuration
    ...
    See [SET](sql-set.md) and
    [Chapter 19](../../server-administration/runtime-config/README.md)
...
:   A string constant defining the function; the meaning depends on the
    ...
    It is often helpful to use dollar quoting (see [Section 4.1.2.4](../../the-sql-language/sql-syntax/sql-syntax-lexical.md#SQL-SYNTAX-DOLLAR-QUOTING)) to write the function definition
...
:   This form of the `AS` clause is used for
    ...
    as for the [`LOAD`](sql-load.md) command. The string
    *`link_symbol`* is the
...
mistrust other users; see [Section 10.3](../../the-sql-language/typeconv/typeconv-func.md).
...
<a id="id-1.9.3.67.10.2"></a><a id="id-1.9.3.67.10.3"></a>

Because a `SECURITY DEFINER` function is executed
with the privileges of the user that owns it, care is needed to
ensure that the function cannot be misused. For security,
[search_path](../../server-administration/runtime-config/runtime-config-client.md#GUC-SEARCH-PATH) should be set to exclude any schemas
...
write `pg_temp`<a id="id-1.9.3.67.10.4.4"></a> as the last entry in `search_path`.
...
(see [Section 5.8](../../the-sql-language/ddl/ddl-priv.md) for more
...
<a id="id-1.9.3.67.12"></a>

## See Also

[ALTER FUNCTION](sql-alterfunction.md), [DROP FUNCTION](sql-dropfunction.md), [GRANT](sql-grant.md), [LOAD](sql-load.md), [REVOKE](sql-revoke.md)

---

原文：[PostgreSQL 18.6 Documentation](https://www.postgresql.org/docs/18/sql-createfunction.html)（英文原文，待翻譯）
