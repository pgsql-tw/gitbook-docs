...
:   This clause specifies optional parameters for a view; the following
    ...
    :   This should be used if the view is intended to provide row-level
        security. See [Section 39.5](../../server-programming/rules/rules-privileges.md) for full details.
    ...
    All of the above options can be changed on existing views using [`ALTER VIEW`](sql-alterview.md).
...
:   This option controls the behavior of automatically updatable views. When
    ...
    `INSERT`, `UPDATE`, and
    `MERGE` commands on the view are
    ...
    with the `INSTEAD` rule. `MERGE` is not
...
Use the [`DROP VIEW`](sql-dropview.md)
...
tables. However, not all views are secure against tampering; see [Section 39.5](../../server-programming/rules/rules-privileges.md) for details.
...
[row-level security](../../the-sql-language/ddl/ddl-rowsecurity.md) enabled, then
...
`INSERT`, `UPDATE`, or
`MERGE` statement attempts to assign a value to it.
...
`DELETE`, and `MERGE`
statements on the view. However, an `UPDATE` or
`MERGE` is allowed to
...
`INSERT`, `UPDATE`, and
`MERGE` commands from creating such rows that are not
...
added. See [Section 39.5](../../server-programming/rules/rules-privileges.md) for full details. Note that,
...
read-only by default: the system will not allow an `INSERT`,
...
on other tables. For more information see [CREATE TRIGGER](sql-createtrigger.md). Another possibility is to create rules
(see [CREATE RULE](sql-createrule.md)), but in practice triggers are
easier to understand and use correctly. Also note that `MERGE`
...
(see [Section 39.5](../../server-programming/rules/rules-privileges.md)). However, if the view has
...
原文：[PostgreSQL 18.6 Documentation](https://www.postgresql.org/docs/18/sql-createview.html)（英文原文，待翻譯）
