...
table. See [Section 11.8](../../the-sql-language/indexes/indexes-partial.md) for more discussion.
...
:   Causes the system to check for
    ...
    partitioned tables; see [CREATE TABLE](sql-createtable.md).
...
:   When this option is used, PostgreSQL will build the
    ...
    — see [Building Indexes Concurrently](sql-createindex.md#SQL-CREATEINDEX-CONCURRENTLY) below.
...
:   The name of the index method to be used. Choices are
    ...
    [bloom](../../appendixes/contrib/bloom.md).
...
`NULLS DISTINCT`<br>`NULLS NOT DISTINCT`
...
:   The name of an index-method-specific storage parameter. See
    [Index Storage Parameters](sql-createindex.md#SQL-CREATEINDEX-STORAGE-PARAMETERS) below
    for details.
...
:   The tablespace in which to create the index. If not specified,
    ...
    [temp_tablespaces](../../server-administration/runtime-config/runtime-config-client.md#GUC-TEMP-TABLESPACES) for indexes on temporary
    tables.
...
parameters* for the index. Each index method has its own set
of allowed storage parameters.

The B-tree, hash, GiST and SP-GiST index methods all accept this
parameter:

<a id="INDEX-RELOPTION-FILLFACTOR"></a>

`fillfactor` (`integer`) <a id="id-1.9.3.69.6.3.4.1.1.3"></a> [#](#INDEX-RELOPTION-FILLFACTOR)
:   Controls how full
    ...
    described in [Section 65.1.4.2](../../internals/indextypes/btree.md#BTREE-DELETION) is dependent on having
...
<a id="INDEX-RELOPTION-DEDUPLICATE-ITEMS"></a>

`deduplicate_items` (`boolean`) <a id="id-1.9.3.69.6.3.6.1.1.3"></a> [#](#INDEX-RELOPTION-DEDUPLICATE-ITEMS)
:   Controls usage of the B-tree deduplication technique described
    in [Section 65.1.4.3](../../internals/indextypes/btree.md#BTREE-DEDUPLICATION). Set to
    `ON` or `OFF` to enable or
    disable the optimization. (Alternative spellings of
    `ON` and `OFF` are allowed as
    described in [Section 19.1](../../server-administration/runtime-config/config-setting.md).) The default is
    `ON`.
...
<a id="INDEX-RELOPTION-BUFFERING"></a>

`buffering` (`enum`) <a id="id-1.9.3.69.6.3.8.1.1.3"></a> [#](#INDEX-RELOPTION-BUFFERING)
:   Controls whether the buffered build technique described in
    [Section 65.2.4.1](../../internals/indextypes/gist.md#GIST-BUFFERING-BUILD) is used to build the index. With
    ...
    [effective_cache_size](../../server-administration/runtime-config/runtime-config-query.md#GUC-EFFECTIVE-CACHE-SIZE). The default
    is `AUTO`.
...
<a id="INDEX-RELOPTION-FASTUPDATE"></a>

`fastupdate` (`boolean`) <a id="id-1.9.3.69.6.3.10.1.1.3"></a> [#](#INDEX-RELOPTION-FASTUPDATE)
:   Controls usage of the fast update technique described in
    [Section 65.4.4.1](../../internals/indextypes/gin.md#GIN-FAST-UPDATE).
    ...
    `VACUUM` the table or call
    the `gin_clean_pending_list`
    function afterward to ensure the pending list is emptied.

<a id="INDEX-RELOPTION-GIN-PENDING-LIST-LIMIT"></a>

`gin_pending_list_limit` (`integer`) <a id="id-1.9.3.69.6.3.11.1.1.3"></a> [#](#INDEX-RELOPTION-GIN-PENDING-LIST-LIMIT)
:   Overrides the global setting of
    [gin_pending_list_limit](../../server-administration/runtime-config/runtime-config-client.md#GUC-GIN-PENDING-LIST-LIMIT) for this index.
    This value is specified in kilobytes.

BRIN indexes accept these parameters:

<a id="INDEX-RELOPTION-PAGES-PER-RANGE"></a>

`pages_per_range` (`integer`) <a id="id-1.9.3.69.6.3.13.1.1.3"></a> [#](#INDEX-RELOPTION-PAGES-PER-RANGE)
:   Defines the number of table blocks that make up one block range for
    each entry of a BRIN index (see [Section 65.5.1](../../internals/indextypes/brin.md#BRIN-INTRO)
    for more details). The default is `128`.
<a id="INDEX-RELOPTION-AUTOSUMMARIZE"></a>

`autosummarize` (`boolean`) <a id="id-1.9.3.69.6.3.13.2.1.3"></a> [#](#INDEX-RELOPTION-AUTOSUMMARIZE)
:   Defines whether a summarization run is queued for the previous page
    range whenever an insertion is detected on the next one
    (see [Section 65.5.1.1](../../internals/indextypes/brin.md#BRIN-OPERATION) for more details).
    The default is `off`.
...
that have a snapshot (see [Chapter 13](../../the-sql-language/mvcc/README.md)) predating the second
...
<a id="id-1.9.3.69.7"></a>

## Notes

See [Chapter 11](../../the-sql-language/indexes/README.md) for information about when indexes can
...
operator classes is in [Section 11.10](../../the-sql-language/indexes/indexes-opclass.md) and in [Section 36.16](../../server-programming/extend/xindex.md).
...
For new expression indexes, it is necessary to run [`ANALYZE`](sql-analyze.md) or wait for
the [autovacuum daemon](../../server-administration/maintenance/routine-vacuuming.md#AUTOVACUUM) to analyze
the table to generate statistics for these indexes.

While `CREATE INDEX` is running, the [search_path](../../server-administration/runtime-config/runtime-config-client.md#GUC-SEARCH-PATH) is temporarily changed to `pg_catalog,
pg_temp`.

For most index methods, the speed of creating an index is
dependent on the setting of [maintenance_work_mem](../../server-administration/runtime-config/runtime-config-resource.md#GUC-MAINTENANCE-WORK-MEM).
...
Increasing [max_parallel_maintenance_workers](../../server-administration/runtime-config/runtime-config-resource.md#GUC-MAX-PARALLEL-MAINTENANCE-WORKERS)
...
Setting a value for `parallel_workers` via [`ALTER TABLE`](sql-altertable.md) directly controls how many parallel
...
Use [`DROP INDEX`](sql-dropindex.md)
to remove an index.
...
view. See [Section 27.4.4](../../server-administration/monitoring/progress-reporting.md#CREATE-INDEX-PROGRESS-REPORTING) for details.

<a id="id-1.9.3.69.8"></a>
...
<a id="id-1.9.3.69.10"></a>

## See Also

[ALTER INDEX](sql-alterindex.md), [DROP INDEX](sql-dropindex.md), [REINDEX](sql-reindex.md), [Section 27.4.4](../../server-administration/monitoring/progress-reporting.md#CREATE-INDEX-PROGRESS-REPORTING)

---

原文：[PostgreSQL 18.6 Documentation](https://www.postgresql.org/docs/18/sql-createindex.html)（英文原文，待翻譯）
