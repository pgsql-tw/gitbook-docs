...
The `CHECKPOINT` command forces an immediate checkpoint when the command is issued, without waiting for a regular checkpoint scheduled by the system (controlled by the settings in [Section 20.5.2](https://www.postgresql.org/docs/15/runtime-config-wal.html#RUNTIME-CONFIG-WAL-CHECKPOINTS)). `CHECKPOINT` is not intended for use during normal operation.
...
Only superusers or users with the privileges of the [`pg_checkpoint`](https://www.postgresql.org/docs/15/predefined-roles.html#PREDEFINED-ROLES-TABLE) role can call `CHECKPOINT`.
...
原文：[PostgreSQL 15.19 Documentation](https://www.postgresql.org/docs/15/sql-checkpoint.html)（英文原文，待翻譯）
