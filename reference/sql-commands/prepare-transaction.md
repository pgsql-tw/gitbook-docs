...
Once prepared, a transaction can later be committed or rolled back with [COMMIT PREPARED](commit-prepared.md) or [ROLLBACK PREPARED](rollback-prepared.md), respectively. Those commands can be issued from any session, not only the one that executed the original transaction.
...
This command must be used inside a transaction block. Use [BEGIN](begin.md) to start one.
