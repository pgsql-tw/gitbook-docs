...
taking regular backups of production databases. See [Chapter 25](../../server-administration/backup/README.md) for further discussion.
...
use [pg_dumpall](app-pg-dumpall.md).

Dumps can be output in script or archive file formats. Script
dumps are plain-text files containing the SQL commands required
to reconstruct the database to the state it was in at the time it was
saved. To restore from such a script, feed it to [psql](app-psql.md). Script files
can be used to reconstruct the database even on other machines and
other architectures; with some modifications, even on other SQL
database products.

The alternative archive file formats must be used with
[pg_restore](app-pgrestore.md) to rebuild the database. They
...
`-b`<br>`--large-objects`<br>`--blobs` (deprecated)
:   Include large objects in the dump. This is the default behavior
    ...
    `--data-only` is used, but not when
    `--schema-only` or `--statistics-only`
    is.

`-B`<br>`--no-large-objects`<br>`--no-blobs` (deprecated)
...
`-e pattern`<br>`--extension=pattern`
:   Dump only extensions matching *`pattern`*. When this option is not
    ...
    [Patterns](app-psql.md#APP-PSQL-PATTERNS)), so multiple extensions can also
...
:   Create the dump in the specified character set encoding. By default,
    ...
    described in [Section 23.3.1](../../server-administration/charset/multibyte.md#MULTIBYTE-CHARSET-SUPPORTED).
...
:   Run the dump in parallel by dumping *`njobs`*
    ...
    + 1 connections to the database, so make sure your [max_connections](../../server-administration/runtime-config/runtime-config-connection.md#GUC-MAX-CONNECTIONS)
    setting is high enough to accommodate all connections.

    Requesting exclusive locks on database objects while running a parallel dump could
    cause the dump to fail. The reason is that the pg_dump leader process
    requests shared locks ([ACCESS SHARE](../../the-sql-language/mvcc/explicit-locking.md#LOCKING-TABLES)) on the
...
:   Dump only schemas matching *`pattern`*; this selects both the
    ...
    *`pattern`* parameter is
    ...
    psql's `\d` commands
    (see [Patterns](app-psql.md#APP-PSQL-PATTERNS)),
    so multiple schemas can also be selected by writing wildcard characters
    in the pattern. When using wildcards, be careful to quote the pattern
    if needed to prevent the shell from expanding the wildcards; see
    [Examples](app-pgdump.md#PG-DUMP-EXAMPLES) below.
...
`-N pattern`<br>`--exclude-schema=pattern`
...
`-Z level`<br>`-Z method`[:*`detail`*]<br>`--compress=level`<br>`--compress=method`[:*`detail`*]
...
:   This option is relevant only when dumping the contents of a table
    ...
    [row_security](../../server-administration/runtime-config/runtime-config-client.md#GUC-ROW-SECURITY) to on instead, allowing the user
...
:   Specify a filename from which to read patterns for objects to include
    ...
    * `schema`: schemas. This works like the
    ...
    Example files are listed below in the [Examples](app-pgdump.md#PG-DUMP-EXAMPLES)
    section.
...
:   Dump the data for any foreign table with a foreign server
    ...
    Using wildcards in `--include-foreign-data` may result
...
:   Use the provided string as the psql
    ...
    specified for plain-text dumps, i.e., when `--format` is
    set to `plain` or the `--format` option
...
:   Use a `serializable` transaction for the dump, to
    ...
    `serialization_failure`. See [Chapter 13](../../the-sql-language/mvcc/README.md)
...
:   Use the specified synchronized snapshot when making a dump of the
    database (see
    [Table 9.100](../../the-sql-language/functions/functions-admin.md#FUNCTIONS-SNAPSHOT-SYNCHRONIZATION-TABLE) for more
    details).

    This option is useful when needing to synchronize the dump with
    a logical replication slot (see [Chapter 47](../../server-programming/logicaldecoding/README.md))
...
:   When set to `fsync`, which is the default,
    ...
    archive directory. See [recovery_init_sync_method](../../server-administration/runtime-config/runtime-config-error-handling.md#GUC-RECOVERY-INIT-SYNC-METHOD)
...
:   Specifies the name of the database to connect to. This is
    ...
    can be a [connection string](../../client-interfaces/libpq/libpq-connect.md#LIBPQ-CONNSTRING).
...
<a id="id-1.9.4.14.7"></a>

## Environment

`PGDATABASE`<br>`PGHOST`<br>`PGOPTIONS`<br>`PGPORT`<br>`PGUSER`
...
select information from the database using, for example, [psql](app-psql.md). Also, any default connection settings and environment
...
When a dump without schema is chosen and the option `--disable-triggers`
...
optimal performance; see [Section 24.1.3](../../server-administration/maintenance/routine-vacuuming.md#VACUUM-FOR-STATISTICS) and [Section 24.1.6](../../server-administration/maintenance/routine-vacuuming.md#AUTOVACUUM) for more information.
...
[`two_phase`](../sql-commands/sql-createsubscription.md#SQL-CREATESUBSCRIPTION-PARAMS-WITH-TWO-PHASE)
...
lower case (see [Patterns](app-psql.md#APP-PSQL-PATTERNS)). But
...
<a id="id-1.9.4.14.11"></a>

## See Also

[pg_dumpall](app-pg-dumpall.md), [pg_restore](app-pgrestore.md), [psql](app-psql.md)

---

原文：[PostgreSQL 18.6 Documentation](https://www.postgresql.org/docs/18/app-pgdump.html)（英文原文，待翻譯）
