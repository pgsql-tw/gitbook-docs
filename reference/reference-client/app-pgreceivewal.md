...
<a id="id-1.9.4.17.4.1"></a>

`pg_receivewal` [*`option`*...]

<a id="id-1.9.4.17.5"></a>
...
for segments to complete like [archive_command](../../server-administration/runtime-config/runtime-config-wal.md#GUC-ARCHIVE-COMMAND) and
[archive_library](../../server-administration/runtime-config/runtime-config-wal.md#GUC-ARCHIVE-LIBRARY) do.
For this reason, it is not necessary to set
[archive_timeout](../../server-administration/runtime-config/runtime-config-wal.md#GUC-ARCHIVE-TIMEOUT) when using
pg_receivewal.
...
[synchronous_commit](../../server-administration/runtime-config/runtime-config-wal.md#GUC-SYNCHRONOUS-COMMIT) equals
...
for [synchronous_standby_names](../../server-administration/runtime-config/runtime-config-replication.md#GUC-SYNCHRONOUS-STANDBY-NAMES), or specify
...
[Section 21.2](../../server-administration/user-manag/role-attributes.md)) or a superuser, and
`pg_hba.conf` must permit the replication connection.
The server must also be configured with
[max_wal_senders](../../server-administration/runtime-config/runtime-config-replication.md#GUC-MAX-WAL-SENDERS) set high enough to leave at least
...
<a id="id-1.9.4.17.6"></a>

## Options

`-D directory`<br>`--directory=directory`
...
:   Require pg_receivewal to use an existing
    replication slot (see [Section 26.2.6](../../server-administration/high-availability/warm-standby.md#STREAMING-REPLICATION-SLOTS)).
...
<a id="id-1.9.4.17.9"></a>

## Notes

When using pg_receivewal instead of
[archive_command](../../server-administration/runtime-config/runtime-config-wal.md#GUC-ARCHIVE-COMMAND) or
[archive_library](../../server-administration/runtime-config/runtime-config-wal.md#GUC-ARCHIVE-LIBRARY) as the main WAL backup method, it is
strongly recommended to use replication slots. Otherwise, the server is
free to recycle or remove write-ahead log files before they are backed up,
because it does not have any information, either
from [archive_command](../../server-administration/runtime-config/runtime-config-wal.md#GUC-ARCHIVE-COMMAND) or
[archive_library](../../server-administration/runtime-config/runtime-config-wal.md#GUC-ARCHIVE-LIBRARY) or the replication slots, about
...
原文：[PostgreSQL 18.6 Documentation](https://www.postgresql.org/docs/18/app-pgreceivewal.html)（英文原文，待翻譯）
