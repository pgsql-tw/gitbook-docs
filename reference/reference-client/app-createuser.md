...
[very extensive privileges](../../server-administration/user-manag/role-attributes.md#ROLE-CREATION).

createuser is a wrapper around the
SQL command [`CREATE ROLE`](../sql-commands/sql-createrole.md).
...
`-c number`<br>`--connection-limit=number`
...
`-g role`<br>`--member-of=role`<br>`--role=role` (deprecated)
...
`-v timestamp`<br>`--valid-until=timestamp`
...
:   The new user will have the `REPLICATION` privilege,
    which is described more fully in the documentation for [CREATE ROLE](../sql-commands/sql-createrole.md).

`--no-replication`
:   The new user will not have the `REPLICATION`
    privilege, which is described more fully in the documentation for [CREATE ROLE](../sql-commands/sql-createrole.md). This is the default.
...
In case of difficulty, see [CREATE ROLE](../sql-commands/sql-createrole.md)
...
<a id="id-1.9.4.5.10"></a>

## See Also

[dropuser](app-dropuser.md), [CREATE ROLE](../sql-commands/sql-createrole.md), [createrole_self_grant](../../server-administration/runtime-config/runtime-config-client.md#GUC-CREATEROLE-SELF-GRANT)

---

原文：[PostgreSQL 18.6 Documentation](https://www.postgresql.org/docs/18/app-createuser.html)（英文原文，待翻譯）
