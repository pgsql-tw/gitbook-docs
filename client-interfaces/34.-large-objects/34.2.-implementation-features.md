...
As of PostgreSQL 9.0, large objects have an owner and a set of access permissions, which can be managed using [GRANT](../../reference/sql-commands/grant.md) and [REVOKE](../../reference/sql-commands/revoke.md). `SELECT` privileges are required to read a large object, and `UPDATE` privileges are required to write or truncate it. Only the large object's owner (or a database superuser) can delete, comment on, or change the owner of a large object. To adjust this behavior for compatibility with prior releases, see the [lo_compat_privileges](https://www.postgresql.org/docs/15/runtime-config-compatible.html#GUC-LO-COMPAT-PRIVILEGES) run-time parameter.

---

原文：[PostgreSQL 15.19 Documentation](https://www.postgresql.org/docs/15/lo-implementation.html)（英文原文，待翻譯）
