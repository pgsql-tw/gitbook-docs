...
`SELECT` statements that return a single result row can also be executed using `EXEC SQL` directly. To handle result sets with multiple rows, an application has to use a cursor; see [Section 36.3.2](#ECPG-CURSORS) below. (As a special case, an application can fetch multiple rows at once into an array host variable; see [Section 36.4.4.3.1](35.4.-using-host-variables.md#ECPG-VARIABLES-ARRAYS).)
...
The tokens of the form <code class="literal">:<em class="replaceable"><code>something</code></em></code> are <em class="firstterm">host variables</em>, that is, they refer to variables in the C program. They are explained in [Section 36.4](35.4.-using-host-variables.md).
...
For more details about declaring a cursor, see [DECLARE](35.14.-embedded-sql-commands/ecpg-sql-declare.md); for more details about fetching rows from a cursor, see [FETCH](../../reference/sql-commands/fetch.md).
...
In the default mode, statements are committed only when `EXEC SQL COMMIT` is issued. The embedded SQL interface also supports autocommit of transactions (similar to psql's default behavior) via the `-t` command-line option to `ecpg` (see [ecpg](../../reference/client-applications/ecpg.md)) or via the `EXEC SQL SET AUTOCOMMIT TO ON` statement. In autocommit mode, each command is automatically committed unless it is inside an explicit transaction block. This mode can be explicitly turned off using `EXEC SQL SET AUTOCOMMIT TO OFF`.
...
`EXEC SQL PREPARE TRANSACTION` <em class="replaceable"><code>transaction&#95;id</code></em>

Prepare the current transaction for two-phase commit.

`EXEC SQL COMMIT PREPARED` <em class="replaceable"><code>transaction&#95;id</code></em>

Commit a transaction that is in prepared state.

`EXEC SQL ROLLBACK PREPARED` <em class="replaceable"><code>transaction&#95;id</code></em>
...
For more details about `PREPARE`, see [PREPARE](35.14.-embedded-sql-commands/ecpg-sql-prepare.md). Also see [Section 36.5](35.5.-dynamic-sql.md) for more details about using placeholders and input parameters.

---

原文：[PostgreSQL 15.19 Documentation](https://www.postgresql.org/docs/15/ecpg-commands.html)（英文原文，待翻譯）
