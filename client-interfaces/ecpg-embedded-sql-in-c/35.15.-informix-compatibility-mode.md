...
`ecpg` can be run in a so-called <em class="firstterm">Informix compatibility mode</em>. If this mode is active, it tries to behave as if it were the Informix precompiler for Informix E/SQL. Generally spoken this will allow you to use the dollar sign instead of the `EXEC SQL` primitive to introduce embedded SQL commands:
...
Informix-compatible mode supports a different structure than the one described in [Section 36.7.2](35.7.-using-descriptor-areas.md#ECPG-SQLDA-DESCRIPTORS). See below:
...
Reserved in Informix, value of [`PQfformat`](https://www.postgresql.org/docs/15/libpq-exec.html#LIBPQ-PQFFORMAT) for the field.
...
Extended type of the field, result of [`PQftype`](https://www.postgresql.org/docs/15/libpq-exec.html#LIBPQ-PQFTYPE).

`sqltypename`<br>`sqltypelen`<br>`sqlownerlen`<br>`sqlsourcetype`<br>`sqlownername`<br>`sqlsourceid`<br>`sqlflags`<br>`sqlreserved`
...
Internally this function uses the [`PGTYPESdate_today`](35.6.-pgtypes-library.md#PGTYPESDATETODAY) function.
...
Internally the function uses the [`PGTYPESdate_julmdy`](35.6.-pgtypes-library.md#PGTYPESDATEJULMDY) function.
...
Internally this function is implemented to use the [`PGTYPESdate_defmt_asc`](35.6.-pgtypes-library.md#PGTYPESDATEDEFMTASC) function. See the reference there for a table of example input.
...
Internally this function uses the [`PGTYPESdate_fmt_asc`](35.6.-pgtypes-library.md#PGTYPESDATEFMTASC) function, see the reference there for examples.
...
Internally the function is implemented to use the function [`PGTYPESdate_mdyjul`](35.6.-pgtypes-library.md#PGTYPESDATEMDYJUL).
...
Internally the function is implemented to use the function [`PGTYPESdate_dayofweek`](35.6.-pgtypes-library.md#PGTYPESDATEDAYOFWEEK).
...
Internally this function uses the [`PGTYPEStimestamp_from_asc`](35.6.-pgtypes-library.md#PGTYPESTIMESTAMPFROMASC) function. See the reference there for a table with example inputs.
...
This function is implemented by means of the [`PGTYPEStimestamp_defmt_asc`](35.6.-pgtypes-library.md#PGTYPESTIMESTAMPDEFMTASC) function. See the documentation there for a list of format specifiers that can be used.
...
Internally, this function uses the [`PGTYPEStimestamp_fmt_asc`](35.6.-pgtypes-library.md#PGTYPESTIMESTAMPFMTASC) function. See the reference there for information on what format mask specifiers can be used.
...
原文：[PostgreSQL 15.19 Documentation](https://www.postgresql.org/docs/15/ecpg-informix-compat.html)（英文原文，待翻譯）
