## 34.3. Running SQL Commands [#](#ECPG-COMMANDS)
...
see [Section 34.3.2](ecpg-commands.md#ECPG-CURSORS) below. (As a special case, an
application can fetch multiple rows at once into an array host
variable; see [Section 34.4.4.3.1](ecpg-variables.md#ECPG-VARIABLES-ARRAYS).)
...
variables in the C program. They are explained in [Section 34.4](ecpg-variables.md).
...
For more details about declaring a cursor, see [DECLARE](ecpg-sql-declare.md); for more details about fetching rows from a
cursor, see [FETCH](../../reference/sql-commands/sql-fetch.md).
...
command-line option to `ecpg` (see [ecpg](../../reference/reference-client/app-ecpg.md)) or via the `EXEC SQL SET AUTOCOMMIT TO
...
see [PREPARE](ecpg-sql-prepare.md). Also
see [Section 34.5](ecpg-dynamic.md) for more details about using
placeholders and input parameters.

---

原文：[PostgreSQL 18.6 Documentation](https://www.postgresql.org/docs/18/ecpg-commands.html)（英文原文，待翻譯）
