## 34.11. Library Functions [#](#ECPG-LIBRARY)
...
* `ECPGdebug(int on, FILE
  *stream)` turns on debug
  ...
  that library.
* `ECPGget_PGconn(const char *connection_name)` returns the library database connection handle identified by the given name.
...
* `ECPGtransactionStatus(const char *connection_name)`
  ...
  See [Section 32.2](../libpq/libpq-status.md) and libpq's [`PQtransactionStatus`](../libpq/libpq-status.md#LIBPQ-PQTRANSACTIONSTATUS) for details about the returned status codes.
