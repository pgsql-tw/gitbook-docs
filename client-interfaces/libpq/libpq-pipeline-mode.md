## 32.5. Pipeline Mode [#](#LIBPQ-PIPELINE-MODE)
...
[Section 54.2.4](../../internals/protocol/protocol-flow.md#PROTOCOL-FLOW-PIPELINING).

<a id="LIBPQ-PIPELINE-USING"></a>

### 32.5.1. Using Pipeline Mode [#](#LIBPQ-PIPELINE-USING)

To issue pipelines, the application must switch the connection
into pipeline mode,
which is done with [`PQenterPipelineMode`](libpq-pipeline-mode.md#LIBPQ-PQENTERPIPELINEMODE).
[`PQpipelineStatus`](libpq-pipeline-mode.md#LIBPQ-PQPIPELINESTATUS) can be used
to test whether pipeline mode is active.
In pipeline mode, only [asynchronous operations](libpq-async.md)
...
to non-pipelined mode with [`PQexitPipelineMode`](libpq-pipeline-mode.md#LIBPQ-PQEXITPIPELINEMODE).

### Note

It is best to use pipeline mode with libpq in
[non-blocking mode](libpq-async.md#LIBPQ-PQSETNONBLOCKING). If used
in blocking mode it is possible for a client/server deadlock to occur.
[<a id="id-1.7.3.12.9.3.1.3"></a>[15]](#ftn.id-1.7.3.12.9.3.1.3)

<a id="LIBPQ-PIPELINE-SENDING"></a>

#### 32.5.1.1. Issuing Queries [#](#LIBPQ-PIPELINE-SENDING)
...
this occurs when [`PQpipelineSync`](libpq-pipeline-mode.md#LIBPQ-PQPIPELINESYNC) is used to
establish a synchronization point in the pipeline,
or when [`PQflush`](libpq-async.md#LIBPQ-PQFLUSH) is called.
The functions [`PQsendPrepare`](libpq-async.md#LIBPQ-PQSENDPREPARE),
...
[`PQsendClosePortal`](libpq-async.md#LIBPQ-PQSENDCLOSEPORTAL) also work in pipeline mode.
...
#### 32.5.1.2. Processing Results [#](#LIBPQ-PIPELINE-RESULTS)
...
queries in the pipeline; see [Section 32.5.1.4](libpq-pipeline-mode.md#LIBPQ-PIPELINE-INTERLEAVE).
...
see [Section 32.5.1.3](libpq-pipeline-mode.md#LIBPQ-PIPELINE-ERRORS).
...
#### 32.5.1.3. Error Handling [#](#LIBPQ-PIPELINE-ERRORS)
...
#### 32.5.1.4. Interleaving Result Processing and Query Dispatch [#](#LIBPQ-PIPELINE-INTERLEAVE)
...
### 32.5.2. Functions Associated with Pipeline Mode [#](#LIBPQ-PIPELINE-FUNCTIONS)

<a id="LIBPQ-PQPIPELINESTATUS"></a>

`PQpipelineStatus`<a id="id-1.7.3.12.10.2.1.1.2"></a> [#](#LIBPQ-PQPIPELINESTATUS)
...
`PQenterPipelineMode`<a id="id-1.7.3.12.10.2.2.1.2"></a> [#](#LIBPQ-PQENTERPIPELINEMODE)
...
`PQexitPipelineMode`<a id="id-1.7.3.12.10.2.3.1.2"></a> [#](#LIBPQ-PQEXITPIPELINEMODE)
:   Causes a connection to exit pipeline mode if it is currently in pipeline mode
    ...
    use [`PQerrorMessage`](libpq-status.md#LIBPQ-PQERRORMESSAGE) to get more information
    about the failure).
<a id="LIBPQ-PQPIPELINESYNC"></a>

`PQpipelineSync`<a id="id-1.7.3.12.10.2.4.1.2"></a> [#](#LIBPQ-PQPIPELINESYNC)
:   Marks a synchronization point in a pipeline by sending a
    [sync message](../../internals/protocol/protocol-flow.md#PROTOCOL-FLOW-EXT-QUERY)
    and flushing the send buffer. This serves as
    the delimiter of an implicit transaction and an error recovery
    point; see [Section 32.5.1.3](libpq-pipeline-mode.md#LIBPQ-PIPELINE-ERRORS).
    ...
    [sync message](../../internals/protocol/protocol-flow.md#PROTOCOL-FLOW-EXT-QUERY)
    failed.
<a id="LIBPQ-PQSENDPIPELINESYNC"></a>

`PQsendPipelineSync`<a id="id-1.7.3.12.10.2.5.1.2"></a> [#](#LIBPQ-PQSENDPIPELINESYNC)
:   Marks a synchronization point in a pipeline by sending a
    [sync message](../../internals/protocol/protocol-flow.md#PROTOCOL-FLOW-EXT-QUERY)
...
`PQsendFlushRequest`<a id="id-1.7.3.12.10.2.6.1.2"></a> [#](#LIBPQ-PQSENDFLUSHREQUEST)
...
### 32.5.3. When to Use Pipeline Mode [#](#LIBPQ-PIPELINE-TIPS)
...
multiple transactions (see [Section 32.5.1.3](libpq-pipeline-mode.md#LIBPQ-PIPELINE-ERRORS)).
...
原文：[PostgreSQL 18.6 Documentation](https://www.postgresql.org/docs/18/libpq-pipeline-mode.html)（英文原文，待翻譯）
