## 32.3. Command Execution Functions [#](#LIBPQ-EXEC)
...
### 32.3.1. Main Functions [#](#LIBPQ-EXEC-MAIN)

<a id="LIBPQ-PQEXEC"></a>

`PQexec`<a id="id-1.7.3.10.3.2.1.1.1.2"></a> [#](#LIBPQ-PQEXEC)
:   Submits a command to the server and waits for the result.
    ...
    the command to the server. The [`PQresultStatus`](libpq-exec.md#LIBPQ-PQRESULTSTATUS) function
    should be called to check the return value for any errors (including
    the value of a null pointer, in which case it will return
    `PGRES_FATAL_ERROR`). Use
    [`PQerrorMessage`](libpq-status.md#LIBPQ-PQERRORMESSAGE) to get more information about such
    errors.

The command string can include multiple SQL commands
(separated by semicolons). Multiple queries sent in a single
[`PQexec`](libpq-exec.md#LIBPQ-PQEXEC) call are processed in a single transaction, unless
there are explicit `BEGIN`/`COMMIT`
commands included in the query string to divide it into multiple
transactions. (See [Section 54.2.2.1](../../internals/protocol/protocol-flow.md#PROTOCOL-FLOW-MULTI-STATEMENT)
...
`PQexecParams`<a id="id-1.7.3.10.3.3.1.1.1.2"></a> [#](#LIBPQ-PQEXECPARAMS)
:   Submits a command to the server and waits for the result,
    ...
    [`PQexecParams`](libpq-exec.md#LIBPQ-PQEXECPARAMS) is like [`PQexec`](libpq-exec.md#LIBPQ-PQEXEC), but offers additional
...
The primary advantage of [`PQexecParams`](libpq-exec.md#LIBPQ-PQEXECPARAMS) over
[`PQexec`](libpq-exec.md#LIBPQ-PQEXEC) is that parameter values can be separated from the
command string, thus avoiding the need for tedious and error-prone
quoting and escaping.

Unlike [`PQexec`](libpq-exec.md#LIBPQ-PQEXEC), [`PQexecParams`](libpq-exec.md#LIBPQ-PQEXECPARAMS) allows at most
...
`PQprepare`<a id="id-1.7.3.10.3.7.1.1.1.2"></a> [#](#LIBPQ-PQPREPARE)
:   Submits a request to create a prepared statement with the
    ...
    [`PQprepare`](libpq-exec.md#LIBPQ-PQPREPARE) creates a prepared statement for later
    execution with [`PQexecPrepared`](libpq-exec.md#LIBPQ-PQEXECPREPARED). This feature allows
    commands to be executed repeatedly without being parsed and
    planned each time; see [PREPARE](../../reference/sql-commands/sql-prepare.md) for details.
    ...
    *`nParams`* is zero.) *`paramTypes[]`*
    ...
    [`PQdescribePrepared`](libpq-exec.md#LIBPQ-PQDESCRIBEPREPARED) for a means to find out
    what data types were inferred.)

    As with [`PQexec`](libpq-exec.md#LIBPQ-PQEXEC), the result is normally a
...
Prepared statements for use with [`PQexecPrepared`](libpq-exec.md#LIBPQ-PQEXECPREPARED) can also
be created by executing SQL [PREPARE](../../reference/sql-commands/sql-prepare.md)
statements.

<a id="LIBPQ-PQEXECPREPARED"></a>

`PQexecPrepared`<a id="id-1.7.3.10.3.8.1.1.1.2"></a> [#](#LIBPQ-PQEXECPREPARED)
:   Sends a request to execute a prepared statement with given
    ...
    [`PQexecPrepared`](libpq-exec.md#LIBPQ-PQEXECPREPARED) is like [`PQexecParams`](libpq-exec.md#LIBPQ-PQEXECPARAMS),
    ...
    The parameters are identical to [`PQexecParams`](libpq-exec.md#LIBPQ-PQEXECPARAMS), except that the
    name of a prepared statement is given instead of a query string, and the
    *`paramTypes[]`* parameter is not present (it is not needed since
    the prepared statement's parameter types were determined when it was created).
<a id="LIBPQ-PQDESCRIBEPREPARED"></a>

`PQdescribePrepared`<a id="id-1.7.3.10.3.8.1.2.1.2"></a> [#](#LIBPQ-PQDESCRIBEPREPARED)
:   Submits a request to obtain information about the specified
    ...
    [`PQdescribePrepared`](libpq-exec.md#LIBPQ-PQDESCRIBEPREPARED) allows an application to obtain
    ...
    *`stmtName`* can be `""` or `NULL` to reference
    ...
    functions [`PQnparams`](libpq-exec.md#LIBPQ-PQNPARAMS) and
    [`PQparamtype`](libpq-exec.md#LIBPQ-PQPARAMTYPE) can be applied to this
    `PGresult` to obtain information about the parameters
    of the prepared statement, and the functions
    [`PQnfields`](libpq-exec.md#LIBPQ-PQNFIELDS), [`PQfname`](libpq-exec.md#LIBPQ-PQFNAME),
    [`PQftype`](libpq-exec.md#LIBPQ-PQFTYPE), etc. provide information about the
    result columns (if any) of the statement.
<a id="LIBPQ-PQDESCRIBEPORTAL"></a>

`PQdescribePortal`<a id="id-1.7.3.10.3.8.1.3.1.2"></a> [#](#LIBPQ-PQDESCRIBEPORTAL)
:   Submits a request to obtain information about the specified
    ...
    [`PQdescribePortal`](libpq-exec.md#LIBPQ-PQDESCRIBEPORTAL) allows an application to obtain
    ...
    *`portalName`* can be `""` or `NULL` to reference
    ...
    [`PQnfields`](libpq-exec.md#LIBPQ-PQNFIELDS), [`PQfname`](libpq-exec.md#LIBPQ-PQFNAME),
    [`PQftype`](libpq-exec.md#LIBPQ-PQFTYPE), etc. can be applied to the
    `PGresult` to obtain information about the result
    columns (if any) of the portal.
<a id="LIBPQ-PQCLOSEPREPARED"></a>

`PQclosePrepared`<a id="id-1.7.3.10.3.8.1.4.1.2"></a> [#](#LIBPQ-PQCLOSEPREPARED)
:   Submits a request to close the specified prepared statement, and waits
    ...
    [`PQclosePrepared`](libpq-exec.md#LIBPQ-PQCLOSEPREPARED) allows an application to close
    ...
    *`stmtName`* can be `""` or
    `NULL` to reference the unnamed statement. It is fine
    if no statement exists with this name, in that case the operation is a
    no-op. On success, a `PGresult` with
    status `PGRES_COMMAND_OK` is returned.
<a id="LIBPQ-PQCLOSEPORTAL"></a>

`PQclosePortal`<a id="id-1.7.3.10.3.8.1.5.1.2"></a> [#](#LIBPQ-PQCLOSEPORTAL)
:   Submits a request to close the specified portal, and waits for
    ...
    [`PQclosePortal`](libpq-exec.md#LIBPQ-PQCLOSEPORTAL) allows an application to trigger
    a close of a previously created portal. Closing a portal releases all
    of its associated resources on the server and allows its name to be
    reused. (libpq does not provide any
    direct access to portals, but you can use this function to close a
    cursor created with a `DECLARE CURSOR` SQL command.)

    *`portalName`* can be `""` or
    `NULL` to reference the unnamed portal. It is fine
    if no portal exists with this name, in that case the operation is a
    no-op. On success, a `PGresult` with status
    `PGRES_COMMAND_OK` is returned.

The `PGresult`<a id="id-1.7.3.10.3.9.2"></a>
...
`PQresultStatus`<a id="id-1.7.3.10.3.9.7.1.1.2"></a> [#](#LIBPQ-PQRESULTSTATUS)
:   Returns the result status of the command.
    ...
    [`PQresultStatus`](libpq-exec.md#LIBPQ-PQRESULTSTATUS) can return one of the following values:
    ...
    :   The `PGresult` contains a single result tuple
        ...
        (see [Section 32.6](libpq-single-row-mode.md)).
    <a id="LIBPQ-PGRES-TUPLES-CHUNK"></a>

    `PGRES_TUPLES_CHUNK` [#](#LIBPQ-PGRES-TUPLES-CHUNK)
    :   The `PGresult` contains several result tuples
        ...
        [`PQsetChunkedRowsMode`](libpq-single-row-mode.md#LIBPQ-PQSETCHUNKEDROWSMODE).
    ...
    :   The `PGresult` represents a
        ...
        [`PQpipelineSync`](libpq-pipeline-mode.md#LIBPQ-PQPIPELINESYNC) or
        [`PQsendPipelineSync`](libpq-pipeline-mode.md#LIBPQ-PQSENDPIPELINESYNC).
    ...
    `PGRES_SINGLE_TUPLE`, or
    `PGRES_TUPLES_CHUNK`, then
    ...
    never be returned directly by [`PQexec`](libpq-exec.md#LIBPQ-PQEXEC) or other
    query execution functions; results of this kind are instead passed
    to the notice processor (see [Section 32.13](libpq-notice-processing.md)).
<a id="LIBPQ-PQRESSTATUS"></a>

`PQresStatus`<a id="id-1.7.3.10.3.9.7.2.1.2"></a> [#](#LIBPQ-PQRESSTATUS)
:   Converts the enumerated type returned by
    [`PQresultStatus`](libpq-exec.md#LIBPQ-PQRESULTSTATUS) into a string constant describing the
...
`PQresultErrorMessage`<a id="id-1.7.3.10.3.9.7.3.1.2"></a> [#](#LIBPQ-PQRESULTERRORMESSAGE)
:   Returns the error message associated with the command, or an empty string
    ...
    passed to [`PQclear`](libpq-exec.md#LIBPQ-PQCLEAR).

    Immediately following a [`PQexec`](libpq-exec.md#LIBPQ-PQEXEC) or
    [`PQgetResult`](libpq-async.md#LIBPQ-PQGETRESULT) call,
    [`PQerrorMessage`](libpq-status.md#LIBPQ-PQERRORMESSAGE) (on the connection) will return
    the same string as [`PQresultErrorMessage`](libpq-exec.md#LIBPQ-PQRESULTERRORMESSAGE) (on
    the result). However, a `PGresult` will
    retain its error message until destroyed, whereas the connection's
    error message will change when subsequent operations are done.
    Use [`PQresultErrorMessage`](libpq-exec.md#LIBPQ-PQRESULTERRORMESSAGE) when you want to
    know the status associated with a particular
    `PGresult`; use
    [`PQerrorMessage`](libpq-status.md#LIBPQ-PQERRORMESSAGE) when you want to know the
    status from the latest operation on the connection.
<a id="LIBPQ-PQRESULTVERBOSEERRORMESSAGE"></a>

`PQresultVerboseErrorMessage`<a id="id-1.7.3.10.3.9.7.4.1.2"></a> [#](#LIBPQ-PQRESULTVERBOSEERRORMESSAGE)
:   Returns a reformatted version of the error message associated with
    ...
    [`PQresultVerboseErrorMessage`](libpq-exec.md#LIBPQ-PQRESULTVERBOSEERRORMESSAGE) addresses this need
    by computing the message that would have been produced
    by [`PQresultErrorMessage`](libpq-exec.md#LIBPQ-PQRESULTERRORMESSAGE) if the specified
...
`PQresultErrorField`<a id="id-1.7.3.10.3.9.7.5.1.2"></a> [#](#LIBPQ-PQRESULTERRORFIELD)
:   Returns an individual field of an error report.
    ...
    *`fieldcode`* is an error field identifier; see the symbols
    ...
    [`PQclear`](libpq-exec.md#LIBPQ-PQCLEAR).
    ...
    `PG_DIAG_SQLSTATE`<a id="id-1.7.3.10.3.9.7.5.2.2.1.3.1.2"></a> [#](#LIBPQ-PG-DIAG-SQLSTATE)
    :   The SQLSTATE code for the error. The SQLSTATE code identifies
        ...
        For a list of the possible SQLSTATE codes, see [Appendix A](../../appendixes/errcodes-appendix/README.md). This field is not localizable,
    ...
    types; see [Appendix A](../../appendixes/errcodes-appendix/README.md). Do not assume that
...
`PQclear`<a id="id-1.7.3.10.3.9.7.6.1.2"></a> [#](#LIBPQ-PQCLEAR)
:   Frees the storage associated with a
    `PGresult`. Every command result should be
    freed via [`PQclear`](libpq-exec.md#LIBPQ-PQCLEAR) when it is no longer
    needed.
    ...
    you must call [`PQclear`](libpq-exec.md#LIBPQ-PQCLEAR). Failure to do this
    will result in memory leaks in your application.

<a id="LIBPQ-EXEC-SELECT-INFO"></a>

### 32.3.2. Retrieving Query Result Information [#](#LIBPQ-EXEC-SELECT-INFO)
...
`PQntuples`<a id="id-1.7.3.10.4.3.1.1.2"></a> [#](#LIBPQ-PQNTUPLES)
...
`PQnfields`<a id="id-1.7.3.10.4.3.2.1.2"></a> [#](#LIBPQ-PQNFIELDS)
...
`PQfname`<a id="id-1.7.3.10.4.3.3.1.2"></a> [#](#LIBPQ-PQFNAME)
...
`PQfnumber`<a id="id-1.7.3.10.4.3.4.1.2"></a> [#](#LIBPQ-PQFNUMBER)
...
`PQftable`<a id="id-1.7.3.10.4.3.5.1.2"></a> [#](#LIBPQ-PQFTABLE)
...
`PQftablecol`<a id="id-1.7.3.10.4.3.6.1.2"></a> [#](#LIBPQ-PQFTABLECOL)
...
`PQfformat`<a id="id-1.7.3.10.4.3.7.1.2"></a> [#](#LIBPQ-PQFFORMAT)
...
`PQftype`<a id="id-1.7.3.10.4.3.8.1.2"></a> [#](#LIBPQ-PQFTYPE)
...
`PQfmod`<a id="id-1.7.3.10.4.3.9.1.2"></a> [#](#LIBPQ-PQFMOD)
...
`PQfsize`<a id="id-1.7.3.10.4.3.10.1.2"></a> [#](#LIBPQ-PQFSIZE)
:   Returns the size in bytes of the column associated with the
    ...
    [`PQfsize`](libpq-exec.md#LIBPQ-PQFSIZE) returns the space allocated for this column
...
`PQbinaryTuples`<a id="id-1.7.3.10.4.3.11.1.2"></a> [#](#LIBPQ-PQBINARYTUPLES)
:   Returns 1 if the `PGresult` contains binary data
    ...
    binary data in others. [`PQfformat`](libpq-exec.md#LIBPQ-PQFFORMAT) is preferred.
    [`PQbinaryTuples`](libpq-exec.md#LIBPQ-PQBINARYTUPLES) returns 1 only if all columns of the
    result are binary (format 1).
<a id="LIBPQ-PQGETVALUE"></a>

`PQgetvalue`<a id="id-1.7.3.10.4.3.12.1.2"></a> [#](#LIBPQ-PQGETVALUE)
:   Returns a single field value of one row of a
    ...
    [`PQgetvalue`](libpq-exec.md#LIBPQ-PQGETVALUE) is a null-terminated character
    ...
    [`PQgetisnull`](libpq-exec.md#LIBPQ-PQGETISNULL) to distinguish null values from
    empty-string values.

    The pointer returned by [`PQgetvalue`](libpq-exec.md#LIBPQ-PQGETVALUE) points
...
`PQgetisnull`<a id="id-1.7.3.10.4.3.13.1.2"></a><a id="id-1.7.3.10.4.3.13.1.3"></a> [#](#LIBPQ-PQGETISNULL)
:   Tests a field for a null value. Row and column numbers start
    ...
    [`PQgetvalue`](libpq-exec.md#LIBPQ-PQGETVALUE) will return an empty string,
    not a null pointer, for a null field.)
<a id="LIBPQ-PQGETLENGTH"></a>

`PQgetlength`<a id="id-1.7.3.10.4.3.14.1.2"></a> [#](#LIBPQ-PQGETLENGTH)
:   Returns the actual length of a field value in bytes. Row and
    ...
    [`PQgetvalue`](libpq-exec.md#LIBPQ-PQGETVALUE). For text data format this is
    the same as `strlen()`. For binary format this is
    essential information. Note that one should *not*
    rely on [`PQfsize`](libpq-exec.md#LIBPQ-PQFSIZE) to obtain the actual data
    length.
<a id="LIBPQ-PQNPARAMS"></a>

`PQnparams`<a id="id-1.7.3.10.4.3.15.1.2"></a> [#](#LIBPQ-PQNPARAMS)
:   Returns the number of parameters of a prepared statement.
    ...
    [`PQdescribePrepared`](libpq-exec.md#LIBPQ-PQDESCRIBEPREPARED). For other types of results it
    will return zero.
<a id="LIBPQ-PQPARAMTYPE"></a>

`PQparamtype`<a id="id-1.7.3.10.4.3.16.1.2"></a> [#](#LIBPQ-PQPARAMTYPE)
...
`PQprint`<a id="id-1.7.3.10.4.3.17.1.2"></a> [#](#LIBPQ-PQPRINT)
...
### 32.3.3. Retrieving Other Result Information [#](#LIBPQ-EXEC-NONSELECT)
...
`PQcmdStatus`<a id="id-1.7.3.10.5.3.1.1.2"></a> [#](#LIBPQ-PQCMDSTATUS)
...
`PQcmdTuples`<a id="id-1.7.3.10.5.3.2.1.2"></a> [#](#LIBPQ-PQCMDTUPLES)
:   Returns the number of rows affected by the SQL command.
    ...
    else, [`PQcmdTuples`](libpq-exec.md#LIBPQ-PQCMDTUPLES) returns an empty string. The caller
    should not free the return value directly. It will be freed when
    the associated `PGresult` handle is passed to
    [`PQclear`](libpq-exec.md#LIBPQ-PQCLEAR).
<a id="LIBPQ-PQOIDVALUE"></a>

`PQoidValue`<a id="id-1.7.3.10.5.3.3.1.2"></a> [#](#LIBPQ-PQOIDVALUE)
:   Returns the OID<a id="id-1.7.3.10.5.3.3.2.1.1"></a>
...
`PQoidStatus`<a id="id-1.7.3.10.5.3.4.1.2"></a> [#](#LIBPQ-PQOIDSTATUS)
:   This function is deprecated in favor of
    [`PQoidValue`](libpq-exec.md#LIBPQ-PQOIDVALUE) and is not thread-safe.
    It returns a string with the OID of the inserted row, while
    [`PQoidValue`](libpq-exec.md#LIBPQ-PQOIDVALUE) returns the OID value.
...
### 32.3.4. Escaping Strings for Inclusion in SQL Commands [#](#LIBPQ-EXEC-ESCAPE-STRING)

<a id="id-1.7.3.10.6.2"></a>

<a id="LIBPQ-PQESCAPELITERAL"></a>

`PQescapeLiteral`<a id="id-1.7.3.10.6.3.1.1.2"></a> [#](#LIBPQ-PQESCAPELITERAL)
:   ```

    char *PQescapeLiteral(PGconn *conn, const char *str, size_t length);
    ```

    [`PQescapeLiteral`](libpq-exec.md#LIBPQ-PQESCAPELITERAL) escapes a string for
    ...
    [`PQescapeLiteral`](libpq-exec.md#LIBPQ-PQESCAPELITERAL) performs this operation.

    [`PQescapeLiteral`](libpq-exec.md#LIBPQ-PQESCAPELITERAL) returns an escaped version of the
    *`str`* parameter in memory allocated with
    ...
    [`PQescapeLiteral`](libpq-exec.md#LIBPQ-PQESCAPELITERAL) stops at the zero; the behavior is
    ...
    On error, [`PQescapeLiteral`](libpq-exec.md#LIBPQ-PQESCAPELITERAL) returns `NULL` and a suitable
    ...
    value is passed as a separate parameter in [`PQexecParams`](libpq-exec.md#LIBPQ-PQEXECPARAMS) or
    its sibling routines.
<a id="LIBPQ-PQESCAPEIDENTIFIER"></a>

`PQescapeIdentifier`<a id="id-1.7.3.10.6.3.2.1.2"></a> [#](#LIBPQ-PQESCAPEIDENTIFIER)
:   ```

    char *PQescapeIdentifier(PGconn *conn, const char *str, size_t length);
    ```

    [`PQescapeIdentifier`](libpq-exec.md#LIBPQ-PQESCAPEIDENTIFIER) escapes a string for
    ...
    [`PQescapeIdentifier`](libpq-exec.md#LIBPQ-PQESCAPEIDENTIFIER) returns a version of the
    *`str`* parameter escaped as an SQL identifier
    ...
    counted in *`length`*. (If a terminating zero byte is found
    before *`length`* bytes are processed,
    [`PQescapeIdentifier`](libpq-exec.md#LIBPQ-PQESCAPEIDENTIFIER) stops at the zero; the behavior is
    thus rather like `strncpy`.) The
    ...
    On error, [`PQescapeIdentifier`](libpq-exec.md#LIBPQ-PQESCAPEIDENTIFIER) returns `NULL` and a suitable
    message is stored in the *`conn`* object.
...
`PQescapeStringConn`<a id="id-1.7.3.10.6.3.3.1.2"></a> [#](#LIBPQ-PQESCAPESTRINGCONN)
:   ```
    ...
    [`PQescapeStringConn`](libpq-exec.md#LIBPQ-PQESCAPESTRINGCONN) escapes string literals, much like
    [`PQescapeLiteral`](libpq-exec.md#LIBPQ-PQESCAPELITERAL). Unlike [`PQescapeLiteral`](libpq-exec.md#LIBPQ-PQESCAPELITERAL),
    the caller is responsible for providing an appropriately sized buffer.
    Furthermore, [`PQescapeStringConn`](libpq-exec.md#LIBPQ-PQESCAPESTRINGCONN) does not generate the
    ...
    *`length`* parameter gives the number of bytes in this
    ...
    counted in *`length`*. (If a terminating zero byte is found
    before *`length`* bytes are processed,
    [`PQescapeStringConn`](libpq-exec.md#LIBPQ-PQESCAPESTRINGCONN) stops at the zero; the behavior is
    ...
    *`conn`* object, whether or not *`error`* is `NULL`.
    ...
    [`PQescapeStringConn`](libpq-exec.md#LIBPQ-PQESCAPESTRINGCONN) returns the number of bytes written
    to *`to`*, not including the terminating zero byte.
<a id="LIBPQ-PQESCAPESTRING"></a>

`PQescapeString`<a id="id-1.7.3.10.6.3.4.1.2"></a> [#](#LIBPQ-PQESCAPESTRING)
:   [`PQescapeString`](libpq-exec.md#LIBPQ-PQESCAPESTRING) is an older, deprecated version of
    ...
    The only difference from [`PQescapeStringConn`](libpq-exec.md#LIBPQ-PQESCAPESTRINGCONN) is that
    [`PQescapeString`](libpq-exec.md#LIBPQ-PQESCAPESTRING) does not take `PGconn`
    or *`error`* parameters.
    ...
    *it might give the wrong results*. Also, it has no way
    ...
    [`PQescapeString`](libpq-exec.md#LIBPQ-PQESCAPESTRING) can be used safely in
...
`PQescapeByteaConn`<a id="id-1.7.3.10.6.3.5.1.2"></a> [#](#LIBPQ-PQESCAPEBYTEACONN)
:   Escapes binary data for use within an SQL command with the type
    `bytea`. As with [`PQescapeStringConn`](libpq-exec.md#LIBPQ-PQESCAPESTRINGCONN),
    ...
    [`PQescapeByteaConn`](libpq-exec.md#LIBPQ-PQESCAPEBYTEACONN) escapes bytes using
    either hex encoding or backslash escaping. See [Section 8.4](../../the-sql-language/datatype/datatype-binary.md) for more information.
    ...
    *`from_length`* parameter gives the number of
    ...
    [`PQescapeByteaConn`](libpq-exec.md#LIBPQ-PQESCAPEBYTEACONN) returns an escaped version of the
...
`PQescapeBytea`<a id="id-1.7.3.10.6.3.6.1.2"></a> [#](#LIBPQ-PQESCAPEBYTEA)
:   [`PQescapeBytea`](libpq-exec.md#LIBPQ-PQESCAPEBYTEA) is an older, deprecated version of
    ...
    The only difference from [`PQescapeByteaConn`](libpq-exec.md#LIBPQ-PQESCAPEBYTEACONN) is that
    [`PQescapeBytea`](libpq-exec.md#LIBPQ-PQESCAPEBYTEA) does not take a `PGconn`
    parameter. Because of this, [`PQescapeBytea`](libpq-exec.md#LIBPQ-PQESCAPEBYTEA) can
    ...
    [`PQescapeByteaConn`](libpq-exec.md#LIBPQ-PQESCAPEBYTEACONN) in such cases).
<a id="LIBPQ-PQUNESCAPEBYTEA"></a>

`PQunescapeBytea`<a id="id-1.7.3.10.6.3.7.1.2"></a> [#](#LIBPQ-PQUNESCAPEBYTEA)
:   Converts a string representation of binary data into binary data
    — the reverse of [`PQescapeBytea`](libpq-exec.md#LIBPQ-PQESCAPEBYTEA). This
    ...
    such as might be returned by [`PQgetvalue`](libpq-exec.md#LIBPQ-PQGETVALUE) when applied
    to a `bytea` column. [`PQunescapeBytea`](libpq-exec.md#LIBPQ-PQUNESCAPEBYTEA)
    converts this string representation into its binary representation.
    It returns a pointer to a buffer allocated with
    `malloc()`, or `NULL` on error, and puts the size of
    the buffer in *`to_length`*. The result must be
    freed using [`PQfreemem`](libpq-misc.md#LIBPQ-PQFREEMEM) when it is no longer needed.

    This conversion is not exactly the inverse of
    [`PQescapeBytea`](libpq-exec.md#LIBPQ-PQESCAPEBYTEA), because the string is not expected
    to be “escaped” when received from [`PQgetvalue`](libpq-exec.md#LIBPQ-PQGETVALUE).
    In particular this means there is no need for string quoting considerations,
    and so no need for a `PGconn` parameter.

---

原文：[PostgreSQL 18.6 Documentation](https://www.postgresql.org/docs/18/libpq-exec.html)（英文原文，待翻譯）
