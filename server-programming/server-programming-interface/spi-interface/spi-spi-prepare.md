<a id="SPI-SPI-PREPARE"></a><a id="id-1.8.12.8.8.1"></a>
...
`SPI_prepare` returns a non-null pointer to an `SPIPlan`, which is an opaque struct representing a prepared statement. On error, `NULL` will be returned, and `SPI_result` will be set to one of the same error codes used by `SPI_execute`, except that it is set to `SPI_ERROR_ARGUMENT` if <em class="parameter"><code>command</code></em> is `NULL`, or if <em class="parameter"><code>nargs</code></em> is less than 0, or if <em class="parameter"><code>nargs</code></em> is greater than 0 and <em class="parameter"><code>argtypes</code></em> is `NULL`.
