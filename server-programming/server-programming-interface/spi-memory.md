...
[SPI_palloc](spi-memory/spi-spi-palloc.md) — allocate memory in the upper executor context

[SPI_repalloc](spi-memory/spi-realloc.md) — reallocate memory in the upper executor context

[SPI_pfree](spi-memory/spi-spi-pfree.md) — free memory in the upper executor context

[SPI_copytuple](spi-memory/spi-spi-copytuple.md) — make a copy of a row in the upper executor context

[SPI_returntuple](spi-memory/spi-spi-returntuple.md) — prepare to return a tuple as a Datum

[SPI_modifytuple](spi-memory/spi-spi-modifytuple.md) — create a row by replacing selected fields of a given row

[SPI_freetuple](spi-memory/spi-spi-freetuple.md) — free a row allocated in the upper executor context

[SPI_freetuptable](spi-memory/spi-spi-freetupletable.md) — free a row set created by `SPI_execute` or a similar function

[SPI_freeplan](spi-memory/spi-spi-freeplan.md) — free a previously saved prepared statement

<a id="id-1.8.12.10.2.1"></a> PostgreSQL allocates memory within <em class="firstterm">memory contexts</em>, which provide a convenient method of managing allocations made in many different places that need to live for differing amounts of time. Destroying a context releases all the memory that was allocated in it. Thus, it is not necessary to keep track of individual objects to avoid memory leaks; instead only a relatively small number of contexts have to be managed. `palloc` and related functions allocate memory from the “current” context.
...
原文：[PostgreSQL 15.19 Documentation](https://www.postgresql.org/docs/15/spi-memory.html)（英文原文，待翻譯）
