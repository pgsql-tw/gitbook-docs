...
[Example 43.3](https://www.postgresql.org/docs/current/plpgsql-trigger.html#PLPGSQL-TRIGGER-EXAMPLE) shows an example of a trigger function in PL/pgSQL.
...
One use of triggers is to maintain a summary table of another table. The resulting summary can be used in place of the original table for certain queries — often with vastly reduced run times. This technique is commonly used in Data Warehousing, where the tables of measured or observed data (called fact tables) might be extremely large. [Example 43.6](https://www.postgresql.org/docs/current/plpgsql-trigger.html#PLPGSQL-TRIGGER-SUMMARY-EXAMPLE) shows an example of a trigger function in PL/pgSQL that maintains a summary table for a fact table in a data warehouse.
...
PL/pgSQL can be used to define [event triggers](https://www.postgresql.org/docs/current/event-triggers.html). PostgreSQL requires that a function that is to be called as an event trigger must be declared as a function with no arguments and a return type of `event_trigger`.
...
[Example 43.8](https://www.postgresql.org/docs/current/plpgsql-trigger.html#PLPGSQL-EVENT-TRIGGER-EXAMPLE) shows an example of an event trigger function in PL/pgSQL.
