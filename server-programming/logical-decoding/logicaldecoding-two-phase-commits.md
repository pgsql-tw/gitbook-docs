...
Optionally the output plugin can define filtering rules via `filter_prepare_cb` to decode only specific transaction in two phases. This can be achieved by pattern matching on the <em class="parameter"><code>gid</code></em> or via lookups using the <em class="parameter"><code>xid</code></em>.
