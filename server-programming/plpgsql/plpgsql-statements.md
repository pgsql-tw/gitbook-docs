## 41.5. Basic Statements [#](#PLPGSQL-STATEMENTS)
...
as described in [Section 41.5.2](plpgsql-statements.md#PLPGSQL-STATEMENTS-GENERAL-SQL).
...
(see [Section 10.4](../../the-sql-language/typeconv/typeconv-query.md)). If no assignment cast is known
...
in [Section 41.5.3](plpgsql-statements.md#PLPGSQL-STATEMENTS-SQL-ONEROW).
To process all of the output rows, write the command as the data
source for a `FOR` loop, as described in
[Section 41.6.6](plpgsql-control-structures.md#PLPGSQL-RECORDS-ITERATING).
...
for expressions; for details see [Section 41.11.1](plpgsql-implementation.md#PLPGSQL-VAR-SUBST).
...
discussed in [Section 41.5.4](plpgsql-statements.md#PLPGSQL-STATEMENTS-EXECUTING-DYN).
...
[Section 41.5.5](plpgsql-statements.md#PLPGSQL-STATEMENTS-DIAGNOSTICS)).
...
[Section 41.5.5](plpgsql-statements.md#PLPGSQL-STATEMENTS-DIAGNOSTICS)) to
...
[Section 41.11.2](plpgsql-implementation.md#PLPGSQL-PLAN-CACHING)) will not work in such
...
[`EXECUTE`](../../reference/sql-commands/sql-execute.md) SQL
...
overview in [Section 41.12.1](plpgsql-development-tips.md#PLPGSQL-QUOTE-TIPS), which can save you
...
`quote_ident` and
`quote_literal` functions (see [Section 9.4](../../the-sql-language/functions/functions-string.md)). For safety, expressions containing column
...
See [Section 9.2](../../the-sql-language/functions/functions-comparison.md) for
...
`format` function (see [Section 9.4.1](../../the-sql-language/functions/functions-string.md#FUNCTIONS-STRING-FORMAT)). For example:
...
`EXECUTE` can be seen in [Example 41.10](plpgsql-porting.md#PLPGSQL-PORTING-EX2), which builds and executes a
...
DIAGNOSTICS` in [Section 41.6.8.1](plpgsql-control-structures.md#PLPGSQL-EXCEPTION-DIAGNOSTICS)).
Each *`item`* is a key word identifying a status
value to be assigned to the specified *`variable`*
(which should be of the right data type to receive it). The currently
available status items are shown
in [Table 41.1](plpgsql-statements.md#PLPGSQL-CURRENT-DIAGNOSTICS-VALUES). Colon-equal
...
<table border="1" class="table" summary="Available Diagnostics Items"><colgroup><col class="col1"/><col class="col2"/><col class="col3"/></colgroup><thead><tr><th>Name</th><th>Type</th><th>Description</th></tr></thead><tbody><tr><td><code class="varname">ROW_COUNT</code></td><td><code class="type">bigint</code></td><td>the number of rows processed by the most
          ...
          (see <a class="xref" href="plpgsql-control-structures.md#PLPGSQL-CALL-STACK">Section 41.6.9</a>)</td></tr><tr><td><code class="literal">PG_ROUTINE_OID</code></td><td><code class="type">oid</code></td><td>OID of the current function</td></tr></tbody></table>
...
原文：[PostgreSQL 18.6 Documentation](https://www.postgresql.org/docs/18/plpgsql-statements.html)（英文原文，待翻譯）
