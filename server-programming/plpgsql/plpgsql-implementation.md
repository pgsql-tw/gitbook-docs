## 41.11. PL/pgSQL under the Hood [#](#PLPGSQL-IMPLEMENTATION)
...
### 41.11.1. Variable Substitution [#](#PLPGSQL-VAR-SUBST)
...
[Section 41.5.4](plpgsql-statements.md#PLPGSQL-STATEMENTS-EXECUTING-DYN).)
...
(see [Section 41.2](plpgsql-structure.md)). For example,
...
`INSERT`, `UPDATE`,
`DELETE`, `MERGE` and commands
containing one of these (such as `EXPLAIN` and
`CREATE TABLE ... AS SELECT`), because the main SQL
...
### 41.11.2. Plan Caching [#](#PLPGSQL-PLAN-CACHING)
...
If it is, generating a plan each time is a net win. See [PREPARE](../../reference/sql-commands/sql-prepare.md) for more information about the behavior of
prepared statements.
...
原文：[PostgreSQL 18.6 Documentation](https://www.postgresql.org/docs/18/plpgsql-implementation.html)（英文原文，待翻譯）
