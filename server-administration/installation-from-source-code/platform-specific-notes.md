...
PostgreSQL 可以使用 Cygwin 建譯，Cygwin 是一種用於 Windows 的類 Linux 環境，但該方法不如原生 Windows 建置（請參閱[第 18 章](broken-reference)），因此並不推薦在 Cygwin 下執行服務。
...
PostgreSQL for Windows 可以使用 MinGW 建置，MinGW 是一種用於 Microsoft 作業系統的類 Unix 執行環境，或是使用 Microsoft 的 Visual C++ 編譯器套件。 MinGW 建置過程使用本章中描述的正常建置系統； Visual C++ 編譯的工作方式則完全不同，將在[第 18 章](broken-reference)中進行描述。
