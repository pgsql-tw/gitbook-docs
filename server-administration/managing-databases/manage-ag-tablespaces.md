...
To define a tablespace, use the [CREATE TABLESPACE](../../reference/sql-commands/sql-createtablespace.md)
command, for example:<a id="id-1.6.9.9.7.2"></a>:
...
Alternatively, use the [default_tablespace](../runtime-config/runtime-config-client.md#GUC-DEFAULT-TABLESPACE) parameter:
...
There is also a [temp_tablespaces](../runtime-config/runtime-config-client.md#GUC-TEMP-TABLESPACES) parameter, which
...
To remove an empty tablespace, use the [DROP TABLESPACE](../../reference/sql-commands/sql-droptablespace.md)
command.

To determine the set of existing tablespaces, examine the
[`pg_tablespace`](../../internals/catalogs/catalog-pg-tablespace.md) system catalog, for example
...
see [Table 9.76](../../the-sql-language/functions/functions-info.md#FUNCTIONS-INFO-CATALOG-TABLE). The [psql](../../reference/reference-client/app-psql.md) program's `\db` meta-command
...
原文：[PostgreSQL 18.6 Documentation](https://www.postgresql.org/docs/18/manage-ag-tablespaces.html)（英文原文，待翻譯）
