...
daemon*, which is described in [Section 24.1.6](routine-vacuuming.md#AUTOVACUUM). You might
...
PostgreSQL's
[`VACUUM`](../../reference/sql-commands/sql-vacuum.md) command has to
...
3. To update the visibility map, which speeds
   up [index-only
   scans](../../the-sql-language/indexes/indexes-index-only-scans.md).
...
[Section 19.10.2](../runtime-config/runtime-config-vacuum.md#RUNTIME-CONFIG-RESOURCE-VACUUM-COST).

<a id="VACUUM-FOR-SPACE-RECOVERY"></a>

### 24.1.2. Recovering Disk Space [#](#VACUUM-FOR-SPACE-RECOVERY)

<a id="id-1.6.11.10.5.2"></a>

In PostgreSQL, an
`UPDATE` or `DELETE` of a row does not
immediately remove the old version of the row.
This approach is necessary to gain the benefits of multiversion
concurrency control (MVCC, see [Chapter 13](../../the-sql-language/mvcc/README.md)): the row version
...
`VACUUM` each one; the program [vacuumdb](../../reference/reference-client/app-vacuumdb.md) might be helpful.
...
[`TRUNCATE`](../../reference/sql-commands/sql-truncate.md) rather
...
<a id="id-1.6.11.10.6.2"></a><a id="id-1.6.11.10.6.3"></a>

The PostgreSQL query planner relies on
statistical information about the contents of tables in order to
generate good plans for queries. These statistics are gathered by
the [`ANALYZE`](../../reference/sql-commands/sql-analyze.md) command,
...
SET STATISTICS`, or change the database-wide default using the [default_statistics_target](../runtime-config/runtime-config-query.md#GUC-DEFAULT-STATISTICS-TARGET) configuration parameter.
...
Vacuum maintains a [visibility map](../../internals/storage/storage-vm.md) for each
...
transaction.
An [*index-only
scan*](../../the-sql-language/indexes/indexes-index-only-scans.md), on the other hand, checks the visibility map first.
...
<a id="id-1.6.11.10.8.2"></a><a id="id-1.6.11.10.8.3"></a>

PostgreSQL's
[MVCC](../../the-sql-language/mvcc/mvcc-intro.md) transaction semantics
...
[vacuum_freeze_min_age](../runtime-config/runtime-config-vacuum.md#GUC-VACUUM-FREEZE-MIN-AGE)
...
`VACUUM` uses the [visibility map](../../internals/storage/storage-vm.md)
...
more all-visible pages by increasing [vacuum_max_eager_freeze_failure_rate](../runtime-config/runtime-config-vacuum.md#GUC-VACUUM-MAX-EAGER-FREEZE-FAILURE-RATE). Even if eager
...
[vacuum_freeze_table_age](../runtime-config/runtime-config-vacuum.md#GUC-VACUUM-FREEZE-TABLE-AGE)
...
XIDs older than the age specified by the configuration parameter [autovacuum_freeze_max_age](../runtime-config/runtime-config-vacuum.md#GUC-AUTOVACUUM-FREEZE-MAX-AGE). (This will happen even if
...
autovacuum logging (controlled by [log_autovacuum_min_duration](../runtime-config/runtime-config-logging.md#GUC-LOG-AUTOVACUUM-MIN-DURATION)) reports on a
...
1. Resolve old prepared transactions. You can find these by checking
   [`pg_prepared_xacts`](../../internals/views/view-pg-prepared-xacts.md) for rows where
   `age(transactionid)` is large. Such transactions should be
   committed or rolled back.
2. End long-running open transactions. You can find these by checking
   [`pg_stat_activity`](../monitoring/monitoring-stats.md#MONITORING-PG-STAT-ACTIVITY-VIEW) for rows where
...
3. Drop any old replication slots. Use
   [`pg_replication_slots`](../../internals/views/view-pg-replication-slots.md) to
   ...
   need to be rebuilt.
4. Execute `VACUUM` in the target database. A database-wide
...
5. Once normal operation is restored, ensure that autovacuum is properly configured
...
[postgres](../../reference/reference-server/app-postgres.md) reference page for details about using single-user mode.

<a id="VACUUM-FOR-MULTIXACT-WRAPAROUND"></a>

#### 24.1.5.1. Multixacts and Wraparound [#](#VACUUM-FOR-MULTIXACT-WRAPAROUND)

<a id="id-1.6.11.10.8.21.2"></a><a id="id-1.6.11.10.8.21.3"></a>
...
[Table 9.84](../../the-sql-language/functions/functions-info.md#FUNCTIONS-PG-SNAPSHOT) can be used to examine the
...
[vacuum_multixact_freeze_min_age](../runtime-config/runtime-config-vacuum.md#GUC-VACUUM-MULTIXACT-FREEZE-MIN-AGE)
...
[vacuum_multixact_freeze_table_age](../runtime-config/runtime-config-vacuum.md#GUC-VACUUM-MULTIXACT-FREEZE-TABLE-AGE), an aggressive
...
Aggressive `VACUUM`s, regardless of what causes
...
occur for any table whose multixact-age is greater than [autovacuum_multixact_freeze_max_age](../runtime-config/runtime-config-vacuum.md#GUC-AUTOVACUUM-MULTIXACT-FREEZE-MAX-AGE). Also, if the
...
1. Running transactions and prepared transactions can be ignored if there
   is no chance that they might appear in a multixact.
2. Unlike transaction ID wraparound, replication slots do not
   ...
   wraparound problems.
3. MXID information is not directly visible in system views such as
...
4. XID exhaustion will block all write transactions, but MXID exhaustion will
...
therefore, autovacuum cannot be used unless [track_counts](../runtime-config/runtime-config-statistics.md#GUC-TRACK-COUNTS) is set to `true`.
...
worker within each database every [autovacuum_naptime](../runtime-config/runtime-config-vacuum.md#GUC-AUTOVACUUM-NAPTIME)
seconds. (Therefore, if the installation has *`N`* databases,
a new worker will be launched every
`autovacuum_naptime`/*`N`* seconds.)
A maximum of [autovacuum_max_workers](../runtime-config/runtime-config-vacuum.md#GUC-AUTOVACUUM-MAX-WORKERS) worker processes
...
[log_autovacuum_min_duration](../runtime-config/runtime-config-logging.md#GUC-LOG-AUTOVACUUM-MIN-DURATION) can be set to monitor
...
workers does not count towards [max_connections](../runtime-config/runtime-config-connection.md#GUC-MAX-CONNECTIONS) or
[superuser_reserved_connections](../runtime-config/runtime-config-connection.md#GUC-SUPERUSER-RESERVED-CONNECTIONS) limits.

Tables whose `relfrozenxid` value is more than
[autovacuum_freeze_max_age](../runtime-config/runtime-config-vacuum.md#GUC-AUTOVACUUM-FREEZE-MAX-AGE) transactions old are always
...
[autovacuum_vacuum_max_threshold](../runtime-config/runtime-config-vacuum.md#GUC-AUTOVACUUM-VACUUM-MAX-THRESHOLD),
the vacuum base threshold is
[autovacuum_vacuum_threshold](../runtime-config/runtime-config-vacuum.md#GUC-AUTOVACUUM-VACUUM-THRESHOLD),
the vacuum scale factor is
[autovacuum_vacuum_scale_factor](../runtime-config/runtime-config-vacuum.md#GUC-AUTOVACUUM-VACUUM-SCALE-FACTOR),
...
[autovacuum_vacuum_insert_threshold](../runtime-config/runtime-config-vacuum.md#GUC-AUTOVACUUM-VACUUM-INSERT-THRESHOLD),
the vacuum insert scale factor is
[autovacuum_vacuum_insert_scale_factor](../runtime-config/runtime-config-vacuum.md#GUC-AUTOVACUUM-VACUUM-INSERT-SCALE-FACTOR),
...
[autovacuum_freeze_min_age](../../reference/sql-commands/sql-createtable.md#RELOPTION-AUTOVACUUM-FREEZE-MIN-AGE) as this may allow
...
[Storage Parameters](../../reference/sql-commands/sql-createtable.md#SQL-CREATETABLE-STORAGE-PARAMETERS) for more information.
...
used. See [Section 19.10.1](../runtime-config/runtime-config-vacuum.md#RUNTIME-CONFIG-AUTOVACUUM) for more details on
the global settings.

When multiple workers are running, the autovacuum cost delay parameters
(see [Section 19.10.2](../runtime-config/runtime-config-vacuum.md#RUNTIME-CONFIG-RESOURCE-VACUUM-COST)) are
...
see [Table 13.2](../../the-sql-language/mvcc/explicit-locking.md#TABLE-LOCK-COMPATIBILITY). However, if the autovacuum
...
原文：[PostgreSQL 18.6 Documentation](https://www.postgresql.org/docs/18/routine-vacuuming.html)（英文原文，待翻譯）
