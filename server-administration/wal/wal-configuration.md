## 28.5. WAL Configuration [#](#WAL-CONFIGURATION)

There are several WAL-related configuration parameters that
affect database performance. This section explains their use.
Consult [Chapter 19](../runtime-config/README.md) for general information about
setting server configuration parameters.

*Checkpoints*<a id="id-1.6.15.7.3.2"></a>
...
a checkpoint every so often. A checkpoint is begun every [checkpoint_timeout](../runtime-config/runtime-config-wal.md#GUC-CHECKPOINT-TIMEOUT) seconds, or if
[max_wal_size](../runtime-config/runtime-config-wal.md#GUC-MAX-WAL-SIZE) is about to be exceeded,
...
adjust the [archive_timeout](../runtime-config/runtime-config-wal.md#GUC-ARCHIVE-TIMEOUT) parameter rather than the
...
[full_page_writes](../runtime-config/runtime-config-wal.md#GUC-FULL-PAGE-WRITES) is set (as is the default), there is
...
parameters, you can set the [checkpoint_warning](../runtime-config/runtime-config-wal.md#GUC-CHECKPOINT-WARNING)
...
[checkpoint_completion_target](../runtime-config/runtime-config-wal.md#GUC-CHECKPOINT-COMPLETION-TARGET), which is
...
On Linux and POSIX platforms [checkpoint_flush_after](../runtime-config/runtime-config-wal.md#GUC-CHECKPOINT-FLUSH-AFTER)
...
[shared_buffers](../runtime-config/runtime-config-resource.md#GUC-SHARED-BUFFERS), but smaller than the OS's page cache.
...
the most recent [wal_keep_size](../runtime-config/runtime-config-replication.md#GUC-WAL-KEEP-SIZE) megabytes of
...
[Section 26.2.6](../high-availability/warm-standby.md#STREAMING-REPLICATION-SLOTS)).
Similarly, if [WAL summarization](../runtime-config/runtime-config-wal.md#RUNTIME-CONFIG-WAL-SUMMARIZATION) is enabled, old segments are kept
until they are summarized.

In archive recovery or standby mode, the server periodically performs
*restartpoints*,<a id="id-1.6.15.7.12.2"></a>
...
if at least [checkpoint_timeout](../runtime-config/runtime-config-wal.md#GUC-CHECKPOINT-TIMEOUT) seconds have passed since
...
but mostly if WAL size is about to exceed [max_wal_size](../runtime-config/runtime-config-wal.md#GUC-MAX-WAL-SIZE)
...
modifying the [wal_buffers](../runtime-config/runtime-config-wal.md#GUC-WAL-BUFFERS) parameter. When
[full_page_writes](../runtime-config/runtime-config-wal.md#GUC-FULL-PAGE-WRITES) is set and the system is very busy,
setting `wal_buffers` higher will help smooth response times
during the period immediately following each checkpoint.

The [commit_delay](../runtime-config/runtime-config-wal.md#GUC-COMMIT-DELAY) parameter defines for how many
...
will occur if [fsync](../runtime-config/runtime-config-wal.md#GUC-FSYNC) is not enabled, or if fewer
than [commit_siblings](../runtime-config/runtime-config-wal.md#GUC-COMMIT-SIBLINGS) other sessions are currently
...
transaction throughput, up to a point. The [pg_test_fsync](../../reference/reference-server/pgtestfsync.md) program can be used to measure the average time
...
The [wal_sync_method](../runtime-config/runtime-config-wal.md#GUC-WAL-SYNC-METHOD) parameter determines how
...
You can test the speeds of different options using the [pg_test_fsync](../../reference/reference-server/pgtestfsync.md) program.
Note that this parameter is irrelevant if `fsync`
has been turned off.

Enabling the [wal_debug](../runtime-config/runtime-config-developer.md#GUC-WAL-DEBUG) configuration parameter
...
When [track_wal_io_timing](../runtime-config/runtime-config-statistics.md#GUC-TRACK-WAL-IO-TIMING) is enabled, the total
amounts of time `XLogWrite` writes and
`issue_xlog_fsync` syncs WAL data to disk are counted as
`write_time` and `fsync_time` in
[pg_stat_io](../monitoring/monitoring-stats.md#PG-STAT-IO-VIEW) for the `object`
`wal`, respectively.
...
The [recovery_prefetch](../runtime-config/runtime-config-wal.md#GUC-RECOVERY-PREFETCH) parameter can be used to reduce
...
The [maintenance_io_concurrency](../runtime-config/runtime-config-resource.md#GUC-MAINTENANCE-IO-CONCURRENCY) and
[wal_decode_buffer_size](../runtime-config/runtime-config-wal.md#GUC-WAL-DECODE-BUFFER-SIZE) settings limit prefetching
concurrency and distance, respectively. By default, it is set to
`try`, which enables the feature on systems that support
issuing read-ahead advice.

---

原文：[PostgreSQL 18.6 Documentation](https://www.postgresql.org/docs/18/wal-configuration.html)（英文原文，待翻譯）
