...
<pre><code><strong>$ initdb -D /usr/local/pgsql/data
</strong></code></pre>
...
Alternatively, you can run `initdb` via the [pg\_ctl](https://www.postgresql.org/docs/current/app-pg-ctl.html) program like so:

<pre><code><strong>$ pg_ctl -D /usr/local/pgsql/data initdb
</strong></code></pre>

This may be more intuitive if you are using `pg_ctl` for starting and stopping the server (see [Section 18.3](https://www.postgresql.org/docs/current/server-start.html)), so that `pg_ctl` would be the sole command you use for managing the database server instance.
...
<pre><code><strong>root# mkdir /usr/local/pgsql
</strong><strong>root# chown postgres /usr/local/pgsql
</strong><strong>root# su postgres
</strong><strong>postgres$ initdb -D /usr/local/pgsql/data
</strong></code></pre>
