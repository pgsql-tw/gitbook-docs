## 18.6. Upgrading a PostgreSQL Cluster [#](#UPGRADING)
...
faster method is [pg_upgrade](../../reference/reference-server/pgupgrade.md). Replication methods are
...
All user-visible changes are listed in the release notes ([Appendix E](../../appendixes/release/README.md)); pay particular attention to the section
...
### 18.6.1. Upgrading Data via pg_dumpall [#](#UPGRADING-VIA-PGDUMPALL)
...
1. If making a backup, make sure that your database is not being updated.
   ...
   See [Chapter 20](../client-authentication/README.md) for additional information on
   access control.
   ...
   command from the version you are currently running; see [Section 25.1.2](../backup/backup-dump.md#BACKUP-DUMP-ALL) for more details. For best
   results, however, try to use the pg_dumpall
   command from PostgreSQL 18.6,
...
2. Shut down the old server:
   ...
   See [Chapter 18](README.md) for details about starting and
   stopping the server.
3. If restoring from backup, rename or delete the old installation
   ...
   remain unchanged.)
4. Install the new version of PostgreSQL as
   outlined in [Chapter 17](../installation/README.md).
5. Create a new database cluster if needed. Remember that you must
   ...
   ```
6. Restore your previous `pg_hba.conf` and any
...
### 18.6.2. Upgrading Data via pg_upgrade [#](#UPGRADING-VIA-PG-UPGRADE)

The [pg_upgrade](../../reference/reference-server/pgupgrade.md) module allows an installation to
...
running initdb. The pg_upgrade [documentation](../../reference/reference-server/pgupgrade.md) outlines the necessary steps.
...
原文：[PostgreSQL 18.6 Documentation](https://www.postgresql.org/docs/18/upgrading.html)（英文原文，待翻譯）
