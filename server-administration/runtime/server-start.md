## 18.3. Starting the Database Server [#](#SERVER-START)
...
for auditing purposes and to diagnose problems. (See [Section 24.3](../maintenance/logfile-maintenance.md) for a more thorough discussion of log
file handling.)

The `postgres` program also takes a number of other
command-line options. For more information, see the
[postgres](../../reference/reference-server/app-postgres.md) reference page
and [Chapter 19](../runtime-config/README.md) below.
...
computer boots.<a id="id-1.6.5.6.8.1"></a>
...
* On Linux systems either add
  ...
  at `/etc/systemd/system/postgresql.service`):<a id="id-1.6.5.6.10.1.3.2.3"></a>
...
buffers ([shared_buffers](../runtime-config/runtime-config-resource.md#GUC-SHARED-BUFFERS)), or
...
([max_connections](../runtime-config/runtime-config-connection.md#GUC-MAX-CONNECTIONS)), but you'll eventually want to
increase the kernel limit.

Details about configuring System V
IPC facilities are given in [Section 18.4.1](kernel-resources.md#SYSVIPC).
...
configure [listen_addresses](../runtime-config/runtime-config-connection.md#GUC-LISTEN-ADDRESSES) so that the server
...
[unix_socket_directories](../runtime-config/runtime-config-connection.md#GUC-UNIX-SOCKET-DIRECTORIES) setting.
...
different message, as shown in [Section 20.16](../client-authentication/client-authentication-problems.md).) Other error messages
...
原文：[PostgreSQL 18.6 Documentation](https://www.postgresql.org/docs/18/server-start.html)（英文原文，待翻譯）
