...
<pre><code><strong>$ postgres -D /usr/local/pgsql/data
</strong></code></pre>
...
<pre><code><strong>$ postgres -D /usr/local/pgsql/data >logfile 2>&#x26;1 &#x26;
</strong></code></pre>

It is important to store the server's stdout and stderr output somewhere, as shown above. It will help for auditing purposes and to diagnose problems. (See [Section 24.3](https://www.postgresql.org/docs/current/logfile-maintenance.html) for a more thorough discussion of log file handling.)

The `postgres` program also takes a number of other command-line options. For more information, see the [postgres](https://www.postgresql.org/docs/current/app-postgres.html) reference page and [Chapter 19](https://www.postgresql.org/docs/current/runtime-config.html) below.

This shell syntax can get tedious quickly. Therefore the wrapper program [pg\_ctl](https://www.postgresql.org/docs/current/app-pg-ctl.html) is provided to simplify some tasks. For example:
...
<pre><code><strong>$ postgres -p 666
</strong>LOG:  could not bind IPv4 address "127.0.0.1": Permission denied
...
Details about configuring System V IPC facilities are given in [Section 18.4.1](https://www.postgresql.org/docs/current/kernel-resources.html#SYSVIPC).
