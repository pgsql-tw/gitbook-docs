## 18.7. Preventing Server Spoofing [#](#PREVENTING-SERVER-SPOOFING)
...
connections is to use a Unix domain socket directory ([unix_socket_directories](../runtime-config/runtime-config-connection.md#GUC-UNIX-SOCKET-DIRECTORIES)) that has write permission only
...
must be configured to accept only `hostssl` connections ([Section 20.1](../client-authentication/auth-pg-hba-conf.md)) and have SSL key and certificate files
([Section 18.9](ssl-tcp.md)). The TCP client must connect using
`sslmode=verify-ca` or
`verify-full` and have the appropriate root certificate
file installed ([Section 32.19.1](../../client-interfaces/libpq/libpq-ssl.md#LIBQ-SSL-CERTIFICATES)). Alternatively the
[system CA pool](../../client-interfaces/libpq/libpq-connect.md#LIBPQ-CONNECT-SSLROOTCERT), as defined
by the SSL implementation, can be used using `sslrootcert=system`; in
...
[scram-sha-256](../client-authentication/auth-password.md) password authentication
...
([Section 20.1](../client-authentication/auth-pg-hba-conf.md)) and use `gss`
authentication with them. The TCP client must connect
using `gssencmode=require`.

---

原文：[PostgreSQL 18.6 Documentation](https://www.postgresql.org/docs/18/preventing-server-spoofing.html)（英文原文，待翻譯）
