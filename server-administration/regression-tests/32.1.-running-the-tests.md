...
or otherwise a note about which tests failed. See [Section 33.2](32.2.-test-evaluation.md) below before assuming that a “failure” represents a serious problem.
...
To run the tests after installation (see [Chapter 17](../installation-from-source-code/README.md)), initialize a data directory and start the server as explained in [Chapter 19](../server-setup-and-operation/README.md), then type:
...
Some of these auxiliary test suites use the TAP infrastructure explained in [Section 33.4](32.4.-tap-tests.md). The TAP-based tests are run only when PostgreSQL was configured with the option `--enable-tap-tests`. This is recommended for development, but can be omitted if there is no suitable Perl installation.
...
This can be useful to enable additional logging, adjust resource limits, or enable extra run-time checks such as [debug\_discard\_caches](../server-configuration/developer-options.md#GUC-DEBUG-DISCARD-CACHES).
