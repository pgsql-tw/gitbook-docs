...
_`name`_ follows the rules for SQL identifiers: either unadorned without special characters, or double-quoted. (In practice, you will usually want to add additional options, such as `LOGIN`, to the command. More details appear below.) To remove an existing role, use the analogous [DROP ROLE](../../reference/sql-commands/drop-role.md) command:

```
DROP ROLE name;
```

For convenience, the programs [createuser](../../reference/client-applications/createuser.md) and [dropuser](../../reference/client-applications/dropuser.md) are provided as wrappers around these SQL commands that can be called from the shell command line:
...
The [psql](../../reference/client-applications/psql.md) program's `\du` meta-command is also useful for listing the existing roles.
