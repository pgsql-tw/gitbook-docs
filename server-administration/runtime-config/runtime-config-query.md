...
constants (see [Section 19.7.2](runtime-config-query.md#RUNTIME-CONFIG-QUERY-CONSTANTS)),
running [`ANALYZE`](../../reference/sql-commands/sql-analyze.md) manually, increasing
the value of the [default_statistics_target](runtime-config-query.md#GUC-DEFAULT-STATISTICS-TARGET) configuration parameter,
...
:   Enables or disables the query planner's ability to reorder DISTINCT
    keys to match the input path's pathkeys. The default is `on`.
...
:   Enables or disables the query planner's use of index-scan and
    ...
    Also see [enable_indexonlyscan](runtime-config-query.md#GUC-ENABLE-INDEXONLYSCAN).
...
:   Enables or disables the query planner's use of index-only-scan plan
    types (see [Section 11.9](../../the-sql-language/indexes/indexes-index-only-scans.md)).
    The default is `on`. The
    [enable_indexscan](runtime-config-query.md#GUC-ENABLE-INDEXSCAN) setting must also be
...
:   Enables or disables the query planner's ability to eliminate a
    ...
    default is `on`.
    See [Section 5.12.4](../../the-sql-language/ddl/ddl-partitioning.md#DDL-PARTITION-PRUNING) for details.
...
:   Sets the planner's estimate of the cost of a disk page fetch
    ...
    (see [ALTER TABLESPACE](../../reference/sql-commands/sql-altertablespace.md)).
...
:   Sets the query cost above which JIT compilation is activated, if
    enabled (see [Chapter 30](../jit/README.md)).
...
For more information see [Chapter 61](../../internals/geqo/README.md).
...
:   Sets the default statistics target for table columns without
    ...
    query planner, refer to [Section 14.2](../../the-sql-language/performance-tips/planner-stats.md).
...
:   Controls the query planner's use of table constraints to
    ...
    [enable_partition_pruning](runtime-config-query.md#GUC-ENABLE-PARTITION-PRUNING).)

    Refer to [Section 5.12.5](../../the-sql-language/ddl/ddl-partitioning.md#DDL-PARTITIONING-CONSTRAINT-EXCLUSION) for
...
:   The planner will merge sub-queries into upper queries if the
    ...
    For more information see [Section 14.3](../../the-sql-language/performance-tips/explicit-joins.md).

    Setting this value to [geqo_threshold](runtime-config-query.md#GUC-GEQO-THRESHOLD) or more
    may trigger use of the GEQO planner, resulting in non-optimal
    plans. See [Section 19.7.3](runtime-config-query.md#RUNTIME-CONFIG-QUERY-GEQO).
<a id="GUC-JIT"></a>

`jit` (`boolean`) <a id="id-1.6.6.10.5.2.5.1.3"></a> [#](#GUC-JIT)
:   Determines whether JIT compilation may be used by
    PostgreSQL, if available (see [Chapter 30](../jit/README.md)).
    The default is `on`.
...
:   Prepared statements (either explicitly prepared or implicitly
    ...
    For more information see [PREPARE](../../reference/sql-commands/sql-prepare.md).
...
:   Sets the planner's estimate of the average size of the working
    ...
    query](../../the-sql-language/queries/queries-with.md#QUERIES-WITH-RECURSIVE), as a multiple of the estimated size of the initial
...
原文：[PostgreSQL 18.6 Documentation](https://www.postgresql.org/docs/18/runtime-config-query.html)（英文原文，待翻譯）
