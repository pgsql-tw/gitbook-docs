## 19.1. Setting Parameters [#](#CONFIG-SETTING)
...
`postgresql.conf`<a id="id-1.6.6.4.3.2.2"></a>,
...
`postgresql.auto.conf`<a id="id-1.6.6.4.3.5.4"></a>,
which has the same format as `postgresql.conf` but
is intended to be edited automatically, not manually. This file holds
settings provided through the [`ALTER SYSTEM`](../../reference/sql-commands/sql-altersystem.md) command.
...
recommended to do this while the server is running unless [allow_alter_system](runtime-config-compatible.md#GUC-ALLOW-ALTER-SYSTEM) is set to `off`, since a
...
* The [`ALTER DATABASE`](../../reference/sql-commands/sql-alterdatabase.md) command allows global
  settings to be overridden on a per-database basis.
* The [`ALTER ROLE`](../../reference/sql-commands/sql-alterrole.md) command allows both global and
...
* The [`SHOW`](../../reference/sql-commands/sql-show.md) command allows inspection of the
  current value of any parameter. The corresponding SQL function is
  `current_setting(setting_name text)`
  (see [Section 9.28.1](../../the-sql-language/functions/functions-admin.md#FUNCTIONS-ADMIN-SET)).
* The [`SET`](../../reference/sql-commands/sql-set.md) command allows modification of the
...
In addition, the system view [`pg_settings`](../../internals/views/view-pg-settings.md) can be
...
原文：[PostgreSQL 18.6 Documentation](https://www.postgresql.org/docs/18/config-setting.html)（英文原文，待翻譯）
