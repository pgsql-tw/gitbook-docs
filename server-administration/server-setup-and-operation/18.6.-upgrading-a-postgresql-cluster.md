...
2.  Shut down the old server:
    ...
    See [Chapter 18](README.md) for details about starting and stopping the server.
...
4. Install the new version of PostgreSQL as outlined in [Section 16.4](../installation-from-source-code/installation-procedure.md).
...
The [pg\_upgrade](../../reference/server-applications/pg_upgrade.md) module allows an installation to be migrated in-place from one major PostgreSQL version to another. Upgrades can be performed in minutes, particularly with `--link` mode. It requires steps similar to pg\_dumpall above, e.g. starting/stopping the server, running initdb. The pg\_upgrade [documentation](../../reference/server-applications/pg_upgrade.md) outlines the necessary steps.
