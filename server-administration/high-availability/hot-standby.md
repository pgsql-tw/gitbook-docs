...
### 26.4.1. User's Overview [#](#HOT-STANDBY-USERS)

When the [hot_standby](../runtime-config/runtime-config-replication.md#GUC-HOT-STANDBY) parameter is set to true on a
...
level. For more details, see [Section 13.2](../../the-sql-language/mvcc/transaction-iso.md).
...
functions ([Table 9.98](../../the-sql-language/functions/functions-admin.md#FUNCTIONS-RECOVERY-INFO-TABLE)) allow users to
...
### 26.4.2. Handling Query Conflicts [#](#HOT-STANDBY-CONFLICT)
...
parameters, [max_standby_archive_delay](../runtime-config/runtime-config-replication.md#GUC-MAX-STANDBY-ARCHIVE-DELAY) and [max_standby_streaming_delay](../runtime-config/runtime-config-replication.md#GUC-MAX-STANDBY-STREAMING-DELAY), that define the maximum
...
visibility map](../maintenance/routine-vacuuming.md#VACUUM-FOR-VISIBILITY-MAP) containing one or more rows
...
is controlled by the [log_recovery_conflict_waits](../runtime-config/runtime-config-logging.md#GUC-LOG-RECOVERY-CONFLICT-WAITS) parameter.

<a id="HOT-STANDBY-ADMIN"></a>

### 26.4.3. Administrator's Overview [#](#HOT-STANDBY-ADMIN)
...
[max_standby_archive_delay](../runtime-config/runtime-config-replication.md#GUC-MAX-STANDBY-ARCHIVE-DELAY) and [max_standby_streaming_delay](../runtime-config/runtime-config-replication.md#GUC-MAX-STANDBY-STREAMING-DELAY). The best choices vary
...
### 26.4.4. Hot Standby Parameter Reference [#](#HOT-STANDBY-PARAMETERS)

Various parameters have been mentioned above in
[Section 26.4.2](hot-standby.md#HOT-STANDBY-CONFLICT) and
[Section 26.4.3](hot-standby.md#HOT-STANDBY-ADMIN).

On the primary, the [wal_level](../runtime-config/runtime-config-wal.md#GUC-WAL-LEVEL) parameter can be used.
[max_standby_archive_delay](../runtime-config/runtime-config-replication.md#GUC-MAX-STANDBY-ARCHIVE-DELAY) and
[max_standby_streaming_delay](../runtime-config/runtime-config-replication.md#GUC-MAX-STANDBY-STREAMING-DELAY) have no effect if set on
the primary.

On the standby, parameters [hot_standby](../runtime-config/runtime-config-replication.md#GUC-HOT-STANDBY),
[max_standby_archive_delay](../runtime-config/runtime-config-replication.md#GUC-MAX-STANDBY-ARCHIVE-DELAY) and
[max_standby_streaming_delay](../runtime-config/runtime-config-replication.md#GUC-MAX-STANDBY-STREAMING-DELAY) can be used.

<a id="HOT-STANDBY-CAVEATS"></a>

### 26.4.5. Caveats [#](#HOT-STANDBY-CAVEATS)
...
* The Serializable transaction isolation level is not yet available in hot
  standby. (See [Section 13.2.3](../../the-sql-language/mvcc/transaction-iso.md#XACT-SERIALIZABLE) and
  [Section 13.4.1](../../the-sql-language/mvcc/applevel-consistency.md#SERIALIZABLE-CONSISTENCY) for details.)
  An attempt to set a transaction to the serializable isolation level in
  hot standby mode will generate an error.

---

原文：[PostgreSQL 18.6 Documentation](https://www.postgresql.org/docs/18/hot-standby.html)（英文原文，待翻譯）
