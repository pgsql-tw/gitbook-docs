...
使用者名映射在標識映射檔中定義，預設情況下名為 pg\_ident.conf，存儲在群集的數據目錄中。（但是，可以將映射檔放置在其他位置;請參閱 [ident\_file](../server-configuration/file-locations.md#ident_file-string) 配置參數。識別對應檔包含一般形式的行：
