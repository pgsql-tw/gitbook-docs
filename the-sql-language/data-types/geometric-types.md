...
幾何資料類型用來描述二維空間物件。[Table 8.20](geometric-types.md#table-8.20.-geometric-types) 列出了 PostgreSQL 中可用的幾何類型。
...
在所有這些型別中，每個座標都是以`double precision`（float8）數字儲存。

A rich set of functions and operators is available to perform various geometric operations such as scaling, translation, rotation, and determining intersections. They are explained in [Section 9.11](https://www.postgresql.org/docs/current/functions-geometry.html).

還有許多的函數和運算子可用於執行各式各樣的幾何運算，例如縮放、平移、旋轉和計算交點。這些內容將在[第 9.11 節](../functions-and-operators/geometric-functions-and-operators.md)中說明。
