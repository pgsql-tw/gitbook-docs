...
tutorial (see [Section 2.1](../../tutorial/tutorial-sql/tutorial-sql-intro.md)), this returns:
...
using a rule (see [Chapter 39](../../server-programming/rules/README.md)). However that does not
...
variant of [`ALTER TABLE`](../../reference/sql-commands/sql-altertable.md).
...
partitioning (see [Section 5.12](ddl-partitioning.md)).
...
`CASCADE` option (see [Section 5.15](ddl-depend.md)).
...
security policies (see [Section 5.9](ddl-rowsecurity.md)) are applied to
...
Foreign tables (see [Section 5.13](ddl-foreign-data.md)) can also
...
原文：[PostgreSQL 18.6 Documentation](https://www.postgresql.org/docs/18/ddl-inherit.html)（英文原文，待翻譯）
