...
* [max_parallel_workers_per_gather](../../server-administration/runtime-config/runtime-config-resource.md#GUC-MAX-PARALLEL-WORKERS-PER-GATHER) must be set to a
...
* The query might be suspended during execution. In any situation in
  ...
  using [DECLARE CURSOR](../../reference/sql-commands/sql-declare.md) will never use
...
* The query uses any function marked `PARALLEL UNSAFE`.
  ...
  [Section 15.4](parallel-safety.md).
...
* No background workers can be obtained because of the limitation that
  ...
  [max_worker_processes](../../server-administration/runtime-config/runtime-config-resource.md#GUC-MAX-WORKER-PROCESSES).
* No background workers can be obtained because of the limitation that
  the total number of background workers launched for purposes of
  parallel query cannot exceed [max_parallel_workers](../../server-administration/runtime-config/runtime-config-resource.md#GUC-MAX-PARALLEL-WORKERS).
* The client sends an Execute message with a non-zero fetch count.
  See the discussion of the
  [extended query protocol](../../internals/protocol/protocol-flow.md#PROTOCOL-FLOW-EXT-QUERY).
  Since [libpq](../../client-interfaces/libpq/README.md) currently provides no way to
  ...
  [max_parallel_workers_per_gather](../../server-administration/runtime-config/runtime-config-resource.md#GUC-MAX-PARALLEL-WORKERS-PER-GATHER) to zero in
  sessions where it is likely, so as to avoid generating query plans
  that may be suboptimal when run serially.

---

原文：[PostgreSQL 18.6 Documentation](https://www.postgresql.org/docs/18/when-can-parallel-query-be-used.html)（英文原文，待翻譯）
