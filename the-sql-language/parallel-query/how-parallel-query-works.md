...
[Using EXPLAIN](../performance-tips/using-explain.md), you can see the number of
...
worker processes](../../server-programming/bgworker/README.md) equal to the number
...
[max_parallel_workers_per_gather](../../server-administration/runtime-config/runtime-config-resource.md#GUC-MAX-PARALLEL-WORKERS-PER-GATHER). The total number
of background workers that can exist at any one time is limited by both
[max_worker_processes](../../server-administration/runtime-config/runtime-config-resource.md#GUC-MAX-WORKER-PROCESSES) and
[max_parallel_workers](../../server-administration/runtime-config/runtime-config-resource.md#GUC-MAX-PARALLEL-WORKERS). Therefore, it is possible for a
...
原文：[PostgreSQL 18.6 Documentation](https://www.postgresql.org/docs/18/how-parallel-query-works.html)（英文原文，待翻譯）
