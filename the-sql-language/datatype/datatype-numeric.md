...
decimals. [Table 8.2](datatype-numeric.md#DATATYPE-NUMERIC-TABLE) lists the
available types.
...
described in [Table 8.2](datatype-numeric.md#DATATYPE-NUMERIC-TABLE).
...
precision to be reduced, the [extra_float_digits](../../server-administration/runtime-config/runtime-config-client.md#GUC-EXTRA-FLOAT-DIGITS)
...
Any value of [extra_float_digits](../../server-administration/runtime-config/runtime-config-client.md#GUC-EXTRA-FLOAT-DIGITS) greater than 0
selects the shortest-precise format.

### Note

Applications that wanted precise values have historically had to set
[extra_float_digits](../../server-administration/runtime-config/runtime-config-client.md#GUC-EXTRA-FLOAT-DIGITS) to 3 to obtain them. For
...
identity column feature, described at [Section 5.3](../ddl/ddl-identity-columns.md).
...
See `nextval()` in [Section 9.17](../functions/functions-sequence.md)
for details.
...
原文：[PostgreSQL 18.6 Documentation](https://www.postgresql.org/docs/18/datatype-numeric.html)（英文原文，待翻譯）
