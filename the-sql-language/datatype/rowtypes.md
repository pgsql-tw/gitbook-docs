...
[*[domain](../../appendixes/glossary/README.md#GLOSSARY-DOMAIN)*](../../appendixes/glossary/README.md#GLOSSARY-DOMAIN) over the composite
...
parentheses. (More details appear [below](rowtypes.md#ROWTYPES-IO-SYNTAX).) Thus, the general format of
...
The `ROW` expression syntax is discussed in more detail in [Section 4.2.13](../sql-syntax/sql-expressions.md#SQL-SYNTAX-ROW-CONSTRUCTORS).
...
further explained in [Section 8.16.5](rowtypes.md#ROWTYPES-USAGE).
...
[above](rowtypes.md#ROWTYPES-DECLARING), we could write:
...
selection](../sql-syntax/sql-expressions.md#FIELD-SELECTION) to the composite value of the table's current row.
...
any composite-valued expression, although as shown [above](rowtypes.md#ROWTYPES-ACCESSING), you need to write parentheses
...
a [`SELECT` output
list](../queries/queries-select-lists.md), a [`RETURNING`
list](../dml/dml-returning.md) in `INSERT`/`UPDATE`/`DELETE`/`MERGE`,
a [`VALUES` clause](../queries/queries-values.md), or
...
in [Section 9.25.6](../functions/functions-comparisons.md#COMPOSITE-TYPE-COMPARISON). However,
...
Dollar quoting (see [Section 4.1.2.4](../sql-syntax/sql-syntax-lexical.md#SQL-SYNTAX-DOLLAR-QUOTING)) can be
...
原文：[PostgreSQL 18.6 Documentation](https://www.postgresql.org/docs/18/rowtypes.html)（英文原文，待翻譯）
