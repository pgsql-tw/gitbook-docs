...
manager subsystem, refer to [Chapter 27](../../server-administration/monitoring/README.md).
...
of these locks explicitly with the command [LOCK](../../reference/sql-commands/sql-lock.md).
...
which each conflicts (see [Table 13.2](explicit-locking.md#TABLE-LOCK-COMPATIBILITY)).
...
:   Conflicts with the `SHARE UPDATE EXCLUSIVE`,
    ...
    and certain [`ALTER INDEX`](../../reference/sql-commands/sql-alterindex.md)
    and [`ALTER TABLE`](../../reference/sql-commands/sql-altertable.md) variants
...
[Table 13.3](explicit-locking.md#ROW-LOCK-COMPATIBILITY) for a complete table of
...
:   `FOR UPDATE` causes the rows retrieved by the
    ...
    [Section 13.4](applevel-consistency.md).
...
currently held by any session can be found in the [`pg_locks`](../../internals/views/view-pg-locks.md) system
view.
...
[max_locks_per_transaction](../../server-administration/runtime-config/runtime-config-locks.md#GUC-MAX-LOCKS-PER-TRANSACTION) and
[max_connections](../../server-administration/runtime-config/runtime-config-connection.md#GUC-MAX-CONNECTIONS).
...
[Section 9.28.10](../functions/functions-admin.md#FUNCTIONS-ADVISORY-LOCKS).

---

原文：[PostgreSQL 18.6 Documentation](https://www.postgresql.org/docs/18/explicit-locking.html)（英文原文，待翻譯）
