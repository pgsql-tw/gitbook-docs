...
The view `pg_locks` provides access to information about the locks held by active processes within the database server. See [Chapter 13](https://www.postgresql.org/docs/12/mvcc.html) for more discussion of locking.
...
| `database` | `oid` | [`pg_database`](https://www.postgresql.org/docs/12/catalog-pg-database.html).oid | OID of the database in which the lock target exists, or zero if the target is a shared object, or null if the target is a transaction ID |
| `relation` | `oid` | [`pg_class`](https://www.postgresql.org/docs/12/catalog-pg-class.html).oid | OID of the relation targeted by the lock, or null if the target is not a relation or part of a relation |
...
| `classid` | `oid` | [`pg_class`](https://www.postgresql.org/docs/12/catalog-pg-class.html).oid | OID of the system catalog containing the lock target, or null if the target is not a general database object |
...
| `mode` | `text` |  | Name of the lock mode held or desired by this process \(see [Section 13.3.1](https://www.postgresql.org/docs/12/explicit-locking.html#LOCKING-TABLES) and [Section 13.2.3](https://www.postgresql.org/docs/12/transaction-iso.html#XACT-SERIALIZABLE)\) |
...
The `pid` column can be joined to the `pid` column of the [`pg_stat_activity`](https://www.postgresql.org/docs/12/monitoring-stats.html#PG-STAT-ACTIVITY-VIEW) view to get more information on the session holding or awaiting each lock, for example
