...
[F.9.1. Example Usage](#id-1.11.7.18.8)

[F.9.2. Authors](#id-1.11.7.18.9)
...
In addition to the typical B-tree search operators, `btree_gist` also provides index support for `<>` (“not equals”). This may be useful in combination with an [exclusion constraint](https://www.postgresql.org/docs/15/sql-createtable.html#SQL-CREATETABLE-EXCLUDE), as described below.
...
Use an [exclusion constraint](https://www.postgresql.org/docs/15/sql-createtable.html#SQL-CREATETABLE-EXCLUDE) to enforce the rule that a cage at a zoo can contain only one kind of animal:
