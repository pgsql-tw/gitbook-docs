...
In addition to the typical B-tree search operators, `btree_gist` also provides index support for `<>` (“not equals”). This may be useful in combination with an [exclusion constraint](../../reference/sql-commands/create-table.md#SQL-CREATETABLE-EXCLUDE), as described below.
...
Use an [exclusion constraint](../../reference/sql-commands/create-table.md#SQL-CREATETABLE-EXCLUDE) to enforce the rule that a cage at a zoo can contain only one kind of animal:
...
原文：[PostgreSQL 15.19 Documentation](btree_gist.md)（英文原文，待翻譯）
