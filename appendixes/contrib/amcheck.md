## F.1. amcheck — tools to verify table and index consistency [#](#AMCHECK)
...
While these checking functions are run, the [search_path](../../server-administration/runtime-config/runtime-config-client.md#GUC-SEARCH-PATH) is temporarily changed to `pg_catalog,
pg_temp`.
...
made with one or more B-Tree support function 1 routines. See [Section 36.16.3](../../server-programming/extend/xindex.md#XINDEX-SUPPORT) for details of operator class support
functions.
...
### F.1.2. Optional *`heapallindexed`* Verification [#](#AMCHECK-OPTIONAL-HEAPALLINDEXED-VERIFICATION)
...
`CREATE INDEX CONCURRENTLY` operation, which checks for the
...
failure modes that [data checksums](../../server-administration/wal/checksums.md) will fail
...
* Structural inconsistencies caused by incorrect operator class
  ...
  *indexed* values might happen to have the same
  ...
  [Section 23.1](../../server-administration/charset/locale.md) and [Section 23.2](../../server-administration/charset/collation.md) for
...
* Corruption caused by hypothetical undiscovered bugs in the
  ...
  when running the standard regression tests. See [Section 31.1](../../server-administration/regress/regress-run.md) for details on running the tests.
...
[data page
checksums](../../server-administration/wal/checksums.md).
...
query could be unsafe on a badly corrupted system. Access to [catalog tables](../../internals/catalogs/catalogs-overview.md) is performed and could
...
an invariant violation should be sought. [pageinspect](pageinspect.md) may play a useful role in diagnosing
corruption that `amcheck` detects. A `REINDEX`
may not be effective in repairing corruption.

---

原文：[PostgreSQL 18.6 Documentation](https://www.postgresql.org/docs/18/amcheck.html)（英文原文，待翻譯）
