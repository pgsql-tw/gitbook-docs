## 68.2. System Catalog Initial Data [#](#SYSTEM-CATALOG-INITIAL-DATA)

[68.2.1. Data File Format](system-catalog-initial-data.md#SYSTEM-CATALOG-INITIAL-DATA-FORMAT)

[68.2.2. OID Assignment](system-catalog-initial-data.md#SYSTEM-CATALOG-OID-ASSIGNMENT)

[68.2.3. OID Reference Lookup](system-catalog-initial-data.md#SYSTEM-CATALOG-OID-REFERENCES)

[68.2.4. Automatic Creation of Array Types](system-catalog-initial-data.md#SYSTEM-CATALOG-AUTO-ARRAY-TYPES)

[68.2.5. Recipes for Editing Data Files](system-catalog-initial-data.md#SYSTEM-CATALOG-RECIPES)
...
### 68.2.1. Data File Format [#](#SYSTEM-CATALOG-INITIAL-DATA-FORMAT)

Each `.dat` file contains Perl data structure literals
...
* Within each catalog row, write comma-separated
  ...
  *`value`* pairs. The
  ...
  is described in [Section 68.2.2](system-catalog-initial-data.md#SYSTEM-CATALOG-OID-ASSIGNMENT) below,
  while `array_type_oid` is described in
  [Section 68.2.4](system-catalog-initial-data.md#SYSTEM-CATALOG-AUTO-ARRAY-TYPES).
...
* All values must be single-quoted. Escape single quotes used within a
  ...
  constants (see [Section 4.1.2.2](../../the-sql-language/sql-syntax/sql-syntax-lexical.md#SQL-SYNTAX-STRINGS-ESCAPE)); for
...
* Null values are represented by `_null_`.
  ...
  string.)
* Comments are preceded by `#`, and must be on their
  own lines.
* Field values that are OIDs of other catalog entries should be
  ...
  This is described in [Section 68.2.3](system-catalog-initial-data.md#SYSTEM-CATALOG-OID-REFERENCES)
  below.
* Since hashes are unordered data structures, field order and line
  ...
  * If the catalog's `.h` file specifies a default
    value for a column, and a data entry has that same
    value, `reformat_dat_file.pl` will omit it from
  ...
  * `reformat_dat_file.pl` preserves blank lines
  ...
  simply change to `src/include/catalog/` and
...
### 68.2.2. OID Assignment [#](#SYSTEM-CATALOG-OID-ASSIGNMENT)
...
### 68.2.3. OID Reference Lookup [#](#SYSTEM-CATALOG-OID-REFERENCES)
...
### 68.2.4. Automatic Creation of Array Types [#](#SYSTEM-CATALOG-AUTO-ARRAY-TYPES)
...
### 68.2.5. Recipes for Editing Data Files [#](#SYSTEM-CATALOG-RECIPES)
...
1. Add the new column, with a default,
   ...
   ```
2. Create a new script based on `reformat_dat_file.pl`
...
4. Remove the old columns from `pg_proc.h`:
   ...
   ```
5. Finally, run `make reformat-dat-files` to remove
...
原文：[PostgreSQL 18.6 Documentation](https://www.postgresql.org/docs/18/system-catalog-initial-data.html)（英文原文，待翻譯）
