## 63.2. Index Access Method Functions [#](#INDEX-FUNCTIONS)
...
the new index.
The `amcanbuildparallel` flag indicates whether
the access method supports parallel index builds. When set to `true`,
...
deferrable; see [Section 63.5](index-unique-checks.md) for details.
...
index insertions, `aminsertcleanup` may be provided,
...
Check whether the index can support [*index-only scans*](../../the-sql-language/indexes/indexes-index-only-scans.md) on
...
in [Section 63.6](index-cost-estimation.md), below.
...
index scanning are described more fully in [Section 63.3](index-scanning.md),
...
scans](../../the-sql-language/indexes/indexes-index-only-scans.md) (i.e., `amcanreturn` returns true for any
of its columns),
...
in [Section 63.3](index-scanning.md).
...
原文：[PostgreSQL 18.6 Documentation](https://www.postgresql.org/docs/18/index-functions.html)（英文原文，待翻譯）
