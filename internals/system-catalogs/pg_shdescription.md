...
The catalog `pg_shdescription` stores optional descriptions (comments) for shared database objects. Descriptions can be manipulated with the [`COMMENT`](../../reference/sql-commands/comment.md) command and viewed with psql's `\d` commands.

See also [`pg_description`](pg_description.md), which performs a similar function for descriptions involving objects within a single database.
...
<strong>Table 53.49. <code class="structname">pg&#95;shdescription</code> Columns</strong>
