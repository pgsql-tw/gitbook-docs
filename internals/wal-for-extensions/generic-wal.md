## 64.1. Generic WAL Records [#](#GENERIC-WAL)
...
Generic WAL records are ignored during [Logical Decoding](../../server-programming/logicaldecoding/README.md). If logical decoding is
...
1. `state = GenericXLogStart(relation)` — start
   construction of a generic WAL record for the given relation.
2. `page = GenericXLogRegisterBuffer(state, buffer, flags)`
...
4. `GenericXLogFinish(state)` — apply the changes to
...
原文：[PostgreSQL 18.6 Documentation](https://www.postgresql.org/docs/18/generic-wal.html)（英文原文，待翻譯）
