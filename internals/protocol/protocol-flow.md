## 54.2. Message Flow [#](#PROTOCOL-FLOW)
...
appear in [Section 54.7](protocol-message-formats.md).) There are
...
### 54.2.1. Start-up [#](#PROTOCOL-FLOW-START-UP)
...
:   The frontend must now initiate a SASL negotiation, using one of the
    ...
    AuthenticationSASLContinue. See [Section 54.3](sasl-authentication.md)
    for details.
...
:   This message informs the frontend about the current (initial)
    setting of backend parameters, such as [client_encoding](../../server-administration/runtime-config/runtime-config-client.md#GUC-CLIENT-ENCODING) or [DateStyle](../../server-administration/runtime-config/runtime-config-client.md#GUC-DATESTYLE).
    The frontend can ignore this message, or record the settings
    for its future use; see [Section 54.2.7](protocol-flow.md#PROTOCOL-ASYNC) for
...
### 54.2.2. Simple Query [#](#PROTOCOL-FLOW-SIMPLE-QUERY)
...
:   The backend is ready to copy data from the frontend to a
    table; see [Section 54.2.6](protocol-flow.md#PROTOCOL-COPY).

CopyOutResponse
:   The backend is ready to copy data from a table to the
    frontend; see [Section 54.2.6](protocol-flow.md#PROTOCOL-COPY).
...
as described in [Section 54.2.6](protocol-flow.md#PROTOCOL-COPY).
...
message. See also [Section 54.2.7](protocol-flow.md#PROTOCOL-ASYNC) concerning messages
...
#### 54.2.2.1. Multiple Statements in a Simple Query [#](#PROTOCOL-FLOW-MULTI-STATEMENT)
...
### 54.2.3. Extended Query [#](#PROTOCOL-FLOW-EXT-QUERY)
...
### 54.2.5. Function Call [#](#PROTOCOL-FLOW-FUNCTION-CALL)
...
### 54.2.6. COPY Operations [#](#PROTOCOL-COPY)
...
[Section 54.2.7](protocol-flow.md#PROTOCOL-ASYNC)). Otherwise, any message type other than
...
in this case. See [Section 54.4](protocol-replication.md) for more
...
### 54.2.7. Asynchronous Operations [#](#PROTOCOL-ASYNC)
...
### 54.2.8. Canceling Requests in Progress [#](#PROTOCOL-FLOW-CANCELING-REQUESTS)
...
### 54.2.9. Termination [#](#PROTOCOL-FLOW-TERMINATION)
...
### 54.2.10. SSL Session Encryption [#](#PROTOCOL-FLOW-SSL)
...
SSL, see [Section 18.9](../../server-administration/runtime/ssl-tcp.md).
...
by [RFC 7301](https://tools.ietf.org/html/rfc7301) to
...
### 54.2.11. GSSAPI Session Encryption [#](#PROTOCOL-FLOW-GSSAPI)
...
able to capture the session traffic. For more information on
encrypting PostgreSQL sessions with
GSSAPI, see [Section 18.10](../../server-administration/runtime/gssapi-enc.md).
...
[RFC 2744](https://datatracker.ietf.org/doc/html/rfc2744)
...
原文：[PostgreSQL 18.6 Documentation](https://www.postgresql.org/docs/18/protocol-flow.html)（英文原文，待翻譯）
