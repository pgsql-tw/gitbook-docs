...
* 原有的 PostQUEL 以 SQL（實作於伺服器端）所取代。（連接介面在 PostQUEL 之後便採 [libpq](../client-interfaces/libpq-c-library/README.md) 函式庫）子查詢一直到 PostgreSQL 出現之前都還未支援，但在 Postgres95 便已能使用自訂的 SQL 函數，聚合函數 Aggregate function 則被重新實作。`GROUP BY` 查詢語句也在此時被加入。
...
更多有關於 PostgreSQL 的發展，請參閱 [附錄 E](../appendixes/release-notes/README.md)。
