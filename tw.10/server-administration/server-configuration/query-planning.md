...
Enables or disables the query planner's use of index-only-scan plan types \(see [Section 11.11](https://www.postgresql.org/docs/10/static/indexes-index-only-scans.html)\). The default is `on`.
