...
3. 更新可視性結構，這會增加[索引限定掃描](../../ii.-sql-cha-xun-yu-yan/11.-suo-yin/11.11.-suo-yin-xian-ding-cha-xun.md)的效率。
...
在 PostgreSQL 中，資料列的 UPDATE 或 DELETE 不會立即刪除該資料列的舊版本。這種方法對於獲得多版本平行控制（MVCC，參閱[第 13 章](../../ii.-sql-cha-xun-yu-yan/13.-yi-zhi-xing-guan-li-mvcc/)）的好處是必要的：資料列的版本不能被刪除，而其他事務仍然可以看到。 但最終，過時或刪除的資料列版本不再讓任何交易感興趣。它佔用的空間必須被新行重新使用以避免無限增長的磁碟空間需求。這是透過執行 VACUUM 來完成的。
