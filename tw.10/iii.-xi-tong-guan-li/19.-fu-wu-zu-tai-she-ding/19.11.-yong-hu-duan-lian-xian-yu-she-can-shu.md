...
另請參閱本頁的 [default\_tablespace](19.11.-yong-hu-duan-lian-xian-yu-she-can-shu.md#defaulttablespacestring)。
...
但是，除非一個模組是專門設計用於非超級用戶的方式，否則這通常不適合使用。請參考使用 [session\_preload\_libraries](19.11.-yong-hu-duan-lian-xian-yu-she-can-shu.md#sessionpreloadlibrariesstring) 參數。
...
與 [shared\_preload\_libraries](19.11.-yong-hu-duan-lian-xian-yu-she-can-shu.md#sharedpreloadlibrariesstring) 不同，在連線啟動時載入函式庫時並沒有很大的效能優勢，相對於第一次使用時。 但是，使用連接池時會有一些優勢。
...
其他的函式庫也可以預先載入。通過預先載入共享函式庫，首次使用函式庫時可以減少啟動時間的成本。但是，啟動每個新伺服器服務的時間可能會略有增加，即使該服務從不使用該函式庫。因此，此參數僅適用於大多數連線中將使用的函式庫。另外，更改此參數需要重新啟動伺服器，因此這不適用於短期除錯事務的需求，請改為使用 [session\_preload\_libraries](19.11.-yong-hu-duan-lian-xian-yu-she-can-shu.md#sessionpreloadlibrariesstring)。
