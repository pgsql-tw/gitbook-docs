...
指定衍生資料表的子查詢必須用圓括號括起來，並且必須分配一個資料表別名（詳見 [7.2.1.2 節](#7212-table-and-column-aliases)）。例如：

```
FROM (SELECT * FROM table1) AS alias_name
```

這例子等同於`FROM table1 AS alias_name`.。當子查詢涉及分組或集合時，會出現更多有趣的情況，這些情況不能簡化為普通的交叉查詢。
...
再一次，一個資料表別名是必須的。將別名分配給 VALUES 列表的欄位是選擇性的，但這是很好的做法。更多訊息請參閱[第 7.7 節](/ii-the-sql-language/queries/77-values-lists.md)。
